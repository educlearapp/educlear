...
const projectId = projectFlag >= 0 ? String(args[projectFlag + 1] || "").trim() : "";
...
function printHuman(audit) {
    ...
    if (audit.sourcePath)
        console.log(`Source:  ${audit.sourcePath}`);
    console.log("");
    ...
    if (audit.balanceVarianceSamples.length) {
        ...
        for (const row of audit.balanceVarianceSamples) {
            console.log(`    ${row.accountNo}: target R${row.target} ledger R${row.ledger} variance R${row.variance}`);
        }
    }
    console.log("");
...
async function main() {
    ...
    const sourcePath = String(filteredArgs[1] || process.env.KIDESYS_CSV_SOURCE || "").trim() || undefined;
    const audit = await (0, daSilvaKidESysCsvImporter_1.auditDaSilvaKidESysCsvImport)({
        schoolId,
        sourcePath,
        projectId: projectId || undefined,
    });
    const outPath = path_1.default.join(process.cwd(), "kideesys-csv-import-audit.json");
