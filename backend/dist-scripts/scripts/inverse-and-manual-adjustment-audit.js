...
const HISTORICAL_MOVEMENT_NOTE = /\b(removed|refund|closed|not returning|relocat|write[\s-]?off|learner left|no longer|cancelled|canceled|credit note|discount|not doing|left school|historical|jamf|paid up|paid-up)\b/i;
...
function buildTimeline(txns, accountNo, familyIndex, duplicateFps) {
    ...
    return sorted.map((t) => {
        ...
        const flags = [];
        if (duplicateFps.has(txnFingerprint(t)))
            flags.push("DUP");
        if (resolveFamilyAccountNo(t, familyIndex) !== accountNo)
            flags.push("MISPOST");
        if (isHistoricalNote(t))
            flags.push("HIST");
        if (isJournalOrCorrection(t))
            flags.push("JNL");
        return {
            date: t.date,
            kind: t.kind,
...
function summarizeTimeline(timeline, section) {
    ...
    const tail = recent
        .map((t) => `${t.date} ${t.kind} R${t.signedAmount}${t.flags.length ? `[${t.flags.join(",")}]` : ""}`)
        .join("; ");
    parts.push(`Recent: ${tail}.`);
    return parts.join(" ");
}
function classifyLikelyCause(opts) {
    const { row, section, txns, ledgerExcess, duplicateInvoiceFps, duplicatePaymentFps, misPostedTxnCount, computedLedger, invoiceSum, paymentSum, } = opts;
    if (row.category === "needsManualKideSysReview" || row.priorActiveMismatchReason === "orphanLedgerOnly") {
        return {
            ...
            detail: row.ageAnalysisBalance < 0.01
...
function assessOpeningBalanceSafety(opts) {
    const { row, cause, causeConfidence, ledgerExcess, ledgerMatchesImport, duplicateInvoiceFps, misPostedTxnCount, section, } = opts;
    ...
    if (row.category === "needsManualKideSysReview" || row.priorActiveMismatchReason === "orphanLedgerOnly") {
        return {
            safe: false,
            safety: "unsafe",
            rationale: "Age is R0 (Paid Up) while ledger still shows R" +
    ...
    if (cause === "extraLedgerChargesNotReflectedInAge" &&
        (causeConfidence === "high" || causeConfidence === "medium") &&
        near(Math.abs(row.adjustmentAmount), ledgerExcess, 2)) {
        return {
            safe: true,
            safety: "safe",
            rationale: "Ledger excess matches identifiable invoice(s) with no dup/mispost signals; opening credit R" +
    ...
    if (cause === "historicalOrSectionSkew" || cause === "ledgerExportOverstatement") {
        return {
            safe: false,
            safety: "caution",
            rationale: `Section "${section}" / export skew — opening credit R${Math.abs(row.adjustmentAmount)} may align EduClear to age but should be confirmed in Kid-e-Sys (age is lower than ledger by design).`,
        };
    }
    if (cause === "cannotDetermine") {
        return {
            safe: false,
            safety: "caution",
            rationale: "Mixed signals on inverse variance — confirm which balance is authoritative before importing opening credit.",
        };
    }
    return {
        safe: false,
...
const targets = [
    ...inverseRows.map((r) => ({
        ...
        variance: r.variance,
        category: "inverseProblemLedgerGreaterThanAge",
        priorActiveMismatchReason: r.priorActiveMismatchReason || "",
    ...
    ...manualRows
        ...
        adjustmentAmount: r.adjustmentAmount,
        variance: r.variance,
        category: "needsManualKideSysReview",
        priorActiveMismatchReason: r.priorActiveMismatchReason || "",
        reasonDetail: r.reasonDetail || "",
    })),
];
...
for (const a of audits)
    causeCounts[a.likelyCause]++;
const safetyCounts = { safe: 0, caution: 0, unsafe: 0 };
for (const a of audits)
    safetyCounts[a.openingBalanceAdjustmentSafety]++;
const report = {
...
const outJson = path_1.default.join(__dirname, "..", "inverse-and-manual-adjustment-audit.json");
const outTxt = path_1.default.join(__dirname, "..", "inverse-and-manual-adjustment-audit.txt");
...
for (const a of audits) {
    const name = String(a.fullName || "")
        .replace(/\n/g, " / ")
        .slice(0, 26);
    const cat = a.category === "inverseProblemLedgerGreaterThanAge" ? "inverse" : "manual";
    lines.push(`${a.accountNo.padEnd(9)} | ${cat.padEnd(8)} | ${a.openingBalanceAdjustmentSafety.padEnd(6)} | ${causeLabels[a.likelyCause].slice(0, 28).padEnd(28)} | ${String(a.ageAnalysisBalance).padStart(5)} | ${String(a.ledgerBalanceFromImport).padStart(6)} | ${String(a.adjustmentAmount).padStart(5)} | ${name}`);
}
...
for (const a of audits) {
    ...
    lines.push(`  opening balance safe: ${a.openingBalanceAdjustmentSafe} [${a.openingBalanceAdjustmentSafety}] — ${a.openingBalanceSafetyRationale}`);
    ...
    for (const t of a.timeline) {
        ...
        lines.push(`    ${t.date} ${t.kind.padEnd(7)} #${t.transactionNo} R${t.signedAmount.toFixed(2)} bal R${t.runningBalance.toFixed(2)}${flagStr}${t.notes ? ` — ${t.notes.slice(0, 55)}` : ""}`);
    }
    lines.push("");
}
