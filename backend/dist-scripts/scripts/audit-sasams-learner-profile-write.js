...
async function main() {
    ...
    const auditPass = active.length > 0 &&
    ...
    console.log(`Home language written: ${active.filter((l) => Boolean(String(l.homeLanguage || "").trim())).length}`);
    console.log(`Citizenship written: ${active.filter((l) => Boolean(String(l.citizenship || "").trim())).length}`);
