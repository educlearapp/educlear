...
async function resolveSchoolId() {
    ...
    if (!top.length)
        throw new Error("No schoolId and no learners in DB");
...
async function main() {
    ...
    const failures = [];
    if (audit.learnersTotal >= MIN_LEARNERS &&
        audit.learnersWithResolvableAccountNo === 0) {
        failures.push(`${audit.learnersTotal} learners but 0 resolvable account numbers`);
    }
    if (audit.ledgerRowsTotal > 0 &&
        audit.kidesysHistoryRowsTotal > 0 &&
        audit.learnersTotal >= MIN_LEARNERS &&
        audit.nonZeroBalanceAccountCount === 0) {
        failures.push(`${audit.learnersTotal} learners with Kid-e-Sys history but all statement balances are R0`);
    }
    if (audit.duplicateStatementAccountKeys > 0) {
        failures.push(`${audit.duplicateStatementAccountKeys} duplicate statement account key(s) (payments list duplication risk)`);
    }
    if (!audit.gatePassed) {
