...
async function main() {
    ...
    if (!fs_1.default.existsSync(sasamsPaths.learnerRegister)) {
    ...
    console.log(`Required SA-SAMS learner count: ${daSilvaMigrationService_1.DA_SILVA_EXPECTED_SASAMS_CLASS_LIST_LEARNER_COUNT}`);
    console.log("Source priority: class_lists (primary) → learner_register (missing fields only)");
    const preDbLearners = await prisma_1.prisma.learner.count({ where: { schoolId } });
    const preDbParents = await prisma_1.prisma.parent.count({ where: { schoolId } });
    const preDbFamilyAccounts = await prisma_1.prisma.familyAccount.count({ where: { schoolId } });
    const preDbLinks = await prisma_1.prisma.parentLearnerLink.count({ where: { schoolId } });
    const preDbClassrooms = await prisma_1.prisma.classroom.count({ where: { schoolId } });
    const preLedgerEntries = (0, billingLedgerStore_1.readSchoolLedger)(schoolId).length;
    ...
    if (preDbFamilyAccounts > 0) {
        console.error("BLOCKED: school already has family accounts. Learners-only import must not run after parents-only.");
        process.exit(1);
    }
    if (preDbLinks > 0) {
        console.error("BLOCKED: school already has parent-learner links. Learners-only import must not run after parents-only.");
        process.exit(1);
    }
    ...
    const preValidation = await (0, daSilvaMigrationService_1.validateDaSilvaLearnersFromKidESys)(sasamsPaths);
    printValidation("Pre-import SA-SAMS validation", preValidation);
    if (!preValidation.passed) {
        process.exit(1);
    }
    const result = await (0, daSilvaMigrationService_1.commitDaSilvaLearnersOnly)({ schoolId, projectId, sasamsPaths });
    ...
    const dbFamilyAccounts = await prisma_1.prisma.familyAccount.count({ where: { schoolId } });
    const dbLinks = await prisma_1.prisma.parentLearnerLink.count({ where: { schoolId } });
    const dbLedgerEntries = (0, billingLedgerStore_1.readSchoolLedger)(schoolId).length;
    const dbBillingPlans = Object.keys((0, learnerBillingPlanStore_1.readSchoolBillingPlans)(schoolId)).length;
    const dbEmployees = await prisma_1.prisma.employee.count({ where: { schoolId } });
    console.log("\n=== Import result ===");
