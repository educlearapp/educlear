...
const kideesysPaths = (0, daSilvaMigrationStrategy_1.resolveDaSilvaKideesysBillingPaths)(desktopRoot);
const paths = {
    classListDir: sasamsPaths.classListDir,
    billingPlan: kideesysPaths.billingPlan,
    ageAnalysis: kideesysPaths.ageAnalysis,
};
function printStagingValidation(label, v) {
    console.log(`\n=== ${label} ===`);
    console.log(`Status: ${v.passed ? "PASS" : "FAIL"}`);
    console.log(`Billing accounts (age analysis): ${v.actualBillingAccounts} (expected ≈ ${v.expectedBillingAccounts})`);
...
async function resolveProjectId(schoolId) {
    ...
    const manifests = fs_1.default
        ...
        .map((f) => ({
        file: f,
        mtime: fs_1.default.statSync(path_1.default.join(stagingRoot, f)).mtimeMs,
    }))
...
async function main() {
    ...
    console.log(`Expected billing accounts: ${daSilvaMigrationService_1.DA_SILVA_EXPECTED_FAMILY_ACCOUNT_COUNT}`);
    const preLedger = (0, billingLedgerStore_1.readSchoolLedger)(schoolId);
    const preBillingPlans = Object.keys((0, learnerBillingPlanStore_1.readSchoolBillingPlans)(schoolId)).length;
    const preFees = await prisma_1.prisma.feeStructure.count({ where: { schoolId } });
    const preEmployees = await prisma_1.prisma.employee.count({ where: { schoolId } });
    const preFamilyAccounts = await prisma_1.prisma.familyAccount.count({ where: { schoolId } });
    ...
    const preStaging = (0, daSilvaMigrationService_1.validateDaSilvaBillingStaging)(paths);
    ...
    let result = null;
    try {
        result = await (0, daSilvaMigrationService_1.commitDaSilvaBillingOnly)({ schoolId, projectId, paths });
    }
    catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        if (!message.includes('phase "billing_accounts" already completed') &&
    ...
    const manifestRaw = JSON.parse(fs_1.default.readFileSync(path_1.default.join(process.cwd(), manifestPath), "utf8"));
    const postImportValidation = await (0, daSilvaMigrationService_1.validateDaSilvaBillingInDatabase)(schoolId, paths, manifestRaw.accountToLearnerId || {});
    const postLedger = (0, billingLedgerStore_1.readSchoolLedger)(schoolId);
    const postTransactionLike = postLedger.filter((e) => e.source !== "kidesys_migration_opening_balance");
    const postEmployees = await prisma_1.prisma.employee.count({ where: { schoolId } });
    console.log("\n=== Import result ===");
    if (result) {
        console.log(`Billing plans imported (learners): ${result.imported.billingPlans}`);
        console.log(`Fee structures: ${result.imported.feeStructuresCreated} created, ${result.imported.feeStructuresExisting} already existed`);
    ...
    console.log(`Kid-e-Sys age analysis total: R${postImportValidation.kidesysAgeAnalysisTotal.toFixed(2)}`);
    console.log(`Zero-balance accounts (Kid-e-Sys has debt): ${postImportValidation.zeroBalanceAccountsWithKidesysDebt.length}`);
    console.log(`Orphan billing refs: ${postImportValidation.orphanBillingAccountRefs.length}`);
    console.log(`Duplicate opening balance refs: ${postImportValidation.duplicateOpeningBalanceRefs.length}`);
    console.log(`Age analysis validation — per-account variance sum: R${postImportValidation.ageAnalysisVarianceTotal.toFixed(2)}`);
    console.log(`Manual opening exclusions: ${daSilvaFinalImportGate_1.DA_SILVA_OPENING_BALANCE_EXCLUDED_ACCOUNTS.join(", ")}`);
    console.log("\n=== Post-import validation ===");
    console.log(`Status: ${postImportValidation.passed ? "PASS" : "FAIL"}`);
    if (postImportValidation.orphanBillingAccountRefs.length) {
        console.log("Orphan refs:", postImportValidation.orphanBillingAccountRefs.slice(0, 20).join(", "));
    }
    if (postImportValidation.zeroBalanceAccountsWithKidesysDebt.length) {
        console.log("Zero ledger but Kid-e-Sys balance:", postImportValidation.zeroBalanceAccountsWithKidesysDebt.slice(0, 20).join(", "));
    }
    if (postImportValidation.errors.length) {
        console.log("Errors:");
        for (const err of postImportValidation.errors)
            console.log(`  - ${err}`);
    }
