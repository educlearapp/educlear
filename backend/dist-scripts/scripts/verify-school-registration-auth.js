...
async function postLogin(loginEmail, loginPassword) {
...
async function postRegisterReclaim(schoolName, contactPerson, phone) {
...
async function main() {
    ...
    if (APPLY_RECLAIM && users.length === 1) {
        ...
        if (school && (0, ownerProvisioning_1.isScriptProvisionedOwner)(u, school)) {
            const reclaim = await postRegisterReclaim(school.name, u.fullName?.includes("Owner") ? "Da Silva Academy" : String(u.fullName || "Owner"), "0000000000");
            const reclaimed = reclaim.status === 200 &&
                reclaim.body &&
                typeof reclaim.body === "object" &&
                reclaim.body.reclaimed;
            add("reclaim", "register-school reclaim (pre-check)", reclaimed ? "pass" : "fail", `status=${reclaim.status} body=${JSON.stringify(reclaim.body)}`);
            if (reclaimed) {
                ...
                if (refreshed)
                    u.passwordHash = refreshed.passwordHash;
                school.email = email;
            }
        }
    }
    for (const user of users) {
        ...
        provisioning = (0, ownerProvisioning_1.describeOwnerProvisioning)(user, school);
        add("provenance", "Owner provisioning source", provisioning === "script" ? "warn" : provisioning === "registration" ? "pass" : "warn", `${provisioning} (fullName=${user.fullName || ""}, school.email=${school.email || "null"})`);
        ...
        add("hash", "bcrypt hash format", hashOk ? "pass" : "fail", `len=${hash.length} prefix=${hash.slice(0, 7)}`);
        passwordMatches = await bcryptjs_1.default.compare(password, hash);
        add("compare", "bcrypt.compare(REGISTRATION_PASSWORD)", passwordMatches ? "pass" : "fail", passwordMatches ? "password matches stored hash" : "password does NOT match stored hash");
        if (provisioning === "script" && !passwordMatches) {
            add("lost", "Registration password never stored", "fail", "Owner was created by migration/repair script — use register-school reclaim (not password reset)");
        }
        const meta = (0, userAccessStore_1.getUserAccessMeta)(user.id);
        add("access", "user-access Owner meta", meta?.appRole === "Owner" ? "pass" : "warn", meta ? `appRole=${meta.appRole}` : "missing — set via register-school or ensure-da-silva-owner");
        const expectedPerms = (0, userPermissions_1.permissionsForRole)("Owner");
        add("perms", "Owner permission map", meta?.permissions && Object.keys(meta.permissions).length >= Object.keys(expectedPerms).length
            ? "pass"
            : "warn", meta?.permissions ? `${Object.keys(meta.permissions).length} modules` : "no meta");
    }
    if (VERIFY_LOGIN || APPLY_RECLAIM) {
        ...
        const token = login.body &&
            ...
            login.body !== null &&
            "token" in login.body
            ? String(login.body.token || "")
            : "";
        loginVerified = login.status === 200 && Boolean(token);
        add("login", "POST /auth/login success", loginVerified ? "pass" : "fail", loginVerified
            ? `200 token issued schoolId=${login.body.user?.schoolId}`
    ...
    const report = {
        ...
        checks,
        recovery: provisioning === "script" && !passwordMatches
    ...
    const outJson = path_1.default.join(process.cwd(), "verify-school-registration-auth.json");
    const outTxt = path_1.default.join(process.cwd(), "verify-school-registration-auth.txt");
    ...
    fs_1.default.writeFileSync(outTxt, txt.join("\n"));
