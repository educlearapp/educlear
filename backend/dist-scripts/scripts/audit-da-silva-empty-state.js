...
function printReport(report) {
    ...
    for (const id of report.scopeSchoolIds)
        console.log(`  - ${id}`);
...
async function main() {
    const report = await (0, daSilvaEmptyState_1.auditDaSilvaEmptyState)(prisma);
    printReport(report);
    if (WRITE_JSON) {
        const jsonOut = path_1.default.join(process.cwd(), "audit-da-silva-empty-state.json");
        const txtOut = path_1.default.join(process.cwd(), "audit-da-silva-empty-state.txt");
        ...
        fs_1.default.writeFileSync(txtOut, lines.join("\n"), "utf8");
        console.log(`\nWrote ${jsonOut}`);
        console.log(`Wrote ${txtOut}`);
    }
    if (!report.passed) {
        const ids = await (0, daSilvaEmptyState_1.collectDaSilvaSchoolIds)(prisma);
