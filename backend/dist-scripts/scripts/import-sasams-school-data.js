...
const importSasamsSchoolData_1 = require("../src/services/sasamsImport/importSasamsSchoolData");
function arg(name) {
...
async function main() {
    ...
    if (!schoolId || !source) {
        console.error("Usage: npx tsx scripts/import-sasams-school-data.ts --schoolId <id> --source <sasamsFolder> [--apply]");
        process.exit(1);
    }
    const paths = (0, importSasamsSchoolData_1.resolveSasamsIngestPaths)(source);
    const dryRun = (0, importSasamsSchoolData_1.dryRunSasamsSchoolImport)(paths);
    ...
    const result = await (0, importSasamsSchoolData_1.importSasamsSchoolData)({
        schoolId,
        paths,
        dryRunOnly: false,
