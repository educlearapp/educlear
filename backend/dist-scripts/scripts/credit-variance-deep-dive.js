...
const JOURNAL_CORRECTION_NOTE = /\b(journal|jnl|correction|correct|adjustment|adj|credit note|cn\b|write[\s-]?off|reversal|reverse|transfer|contra)\b/i;
...
function classifyCreditLedgerCause(opts) {
    const { row, section, txns, duplicateFingerprints, misPostedTxnCount, crossAccountInbound, crossAccountOutbound, siblingCombinedVariance, familyVarianceImprovement, mergedFamilySignals, historicalOnlyLedger, duplicateNameCount, } = opts;
    ...
    if (familyVarianceImprovement !== null &&
    ...
    const paymentsWithoutInvoice = (invoiceCount === 0 && paymentCount > 0) ||
        (paymentCount > 0 &&
            invoiceSum > 0.01 &&
    ...
    if (paymentsWithoutInvoice) {
        return {
            ...
            confidence: invoiceCount === 0 || near(lifetimeOverpayment, Math.abs(row.ledgerBalanceFromImport), 50)
                ? "high"
                : "medium",
    ...
    const historicalPaymentCount = txns.filter((t) => t.kind === "payment" && isHistoricalNote(t)).length;
    if (historicalPaymentCount > 0 &&
...
function buildSerializedTimeline(txns, accountNo, familyIndex, duplicateFps) {
    ...
    const timeline = sorted.map((t) => {
        ...
        const misPosted = resolveFamilyAccountNo(t, familyIndex) !== accountNo && (0, kideesysSpreadsheet_1.normalizeMatchText)(t.fullName) !== "";
        const row = {
            date: t.date,
            kind: t.kind,
    ...
    const journalCorrectionCreditRows = timeline.filter((t) => t.flags.journalOrCorrection || t.flags.historicalNote || (t.kind === "payment" && t.signedAmount > 0));
...
const targets = (review.accountsByReasonGroup?.possibleDuplicatedHistoricalCredit || []).map((r) => ({
...
for (const row of targets) {
    ...
    const siblingAccountNos = [...(parentSiblings.get(row.accountNo) || new Set())];
    let siblingCombinedVariance = null;
    let familyVarianceImprovement = null;
    ...
    const mergedFamilySignals = [];
    if (mergedFamilyAccountNos.has(row.accountNo))
        mergedFamilySignals.push("mergedFamilyAccountNos");
    if ((0, daSilvaMergedFamily_1.splitMergedAccountNames)(row.fullName).length > 1)
        mergedFamilySignals.push("splitMergedAccountName");
    if ((0, daSilvaMergedFamily_1.hasSilentBillingSibling)(row.accountNo, familyIndex, bundle.transactions)) {
    ...
    const { invoices, payments, journalCorrectionCreditRows, timeline } = buildSerializedTimeline(txns, row.accountNo, familyIndex, duplicateFps);
...
for (const d of deepDives)
    causeCounts[d.negativeCreditLedgerCause]++;
const report = {
...
const outJson = path_1.default.join(__dirname, "..", "credit-variance-deep-dive.json");
const outTxt = path_1.default.join(__dirname, "..", "credit-variance-deep-dive.txt");
...
for (const d of deepDives) {
    ...
    lines.push(`  invoices: ${d.metrics.invoiceCount} (R${d.metrics.invoiceSum}) | payments: ${d.metrics.paymentCount} (R${d.metrics.paymentSum}) | journal/credit flagged: ${d.journalCorrectionCreditRows.length}`);
    ...
    if (d.metrics.siblingAccountNos.length) {
        lines.push(`  siblings: ${d.metrics.siblingAccountNos.join(", ")} | combined variance: ${d.metrics.siblingCombinedVariance}`);
    }
    lines.push("  timeline:");
    for (const t of d.timeline) {
        const flags = [];
        if (t.flags.duplicateFingerprint)
            flags.push("DUP");
        if (t.flags.misPostedByLearnerName)
            flags.push("MISPOST");
        if (t.flags.historicalNote)
            flags.push("HIST");
        if (t.flags.journalOrCorrection)
            flags.push("JNL");
        const flagStr = flags.length ? ` [${flags.join(",")}]` : "";
        lines.push(`    ${t.date} ${t.kind.padEnd(7)} #${t.transactionNo} R${t.signedAmount.toFixed(2)} bal R${t.runningBalance.toFixed(2)}${flagStr} ${t.notes ? `— ${t.notes.slice(0, 60)}` : ""}`);
    }
    lines.push("");
}
