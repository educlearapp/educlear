...
const daSilvaOpeningBalance_1 = require("../src/services/daSilvaMigration/daSilvaOpeningBalance");
...
const FREEZE_EXCLUDED_MANUAL = [{ accountNo: "MAR005", status: "manual reconciliation required" }];
const EXCLUDED_ACCOUNT_NOS = new Set(FREEZE_EXCLUDED_MANUAL.map((e) => e.accountNo));
function applyFreezeOpeningBalancePlan(adjustments) {
...
const bundle = (0, daSilvaMigrationService_1.buildDaSilvaBundleFromDesktopLayout)("freeze-preview", "freeze-preview", desktopRoot);
...
const ageAnalysisRemainingVariance = (0, daSilvaOpeningBalance_1.countAgeAnalysisVarianceAfterAdjustments)(bundle.reconciliation.rows, freezeAdjustments, ageAnalysisAccountNos);
const mergedFamilyLedgerGaps = (0, daSilvaFinalImportGate_1.countMergedFamilyLedgerGaps)(bundle);
const missingInverse = FREEZE_INCLUDED_INVERSE.filter((acct) => !freezeAdjustments.some((a) => a.accountNo === acct));
const stillHasMar005 = freezeAdjustments.some((a) => a.accountNo === "MAR005");
const excludedManualAccounts = FREEZE_EXCLUDED_MANUAL.map((rule) => {
...
const structuralOk = bundle.canImport &&
...
console.log(`Opening balance adjustment count: ${freezeAdjustments.length} (base plan: ${bundle.openingBalance.summary.adjustmentCount}, excluded manual: ${bundle.openingBalance.summary.adjustmentCount - freezeAdjustments.length})`);
...
console.log(`Excluded manual accounts: ${excludedManualAccounts.map((e) => `${e.accountNo} (${e.status})`).join(", ") || "none"}`);
for (const ex of excludedManualAccounts) {
    console.log(`  ${ex.accountNo} ${ex.fullName}: age R${ex.ageAnalysisBalance.toFixed(2)}, ledger R${ex.ledgerBalanceFromImport.toFixed(2)}, variance R${ex.variance.toFixed(2)} — ${ex.status}`);
}
...
for (const acct of FREEZE_INCLUDED_INVERSE) {
    ...
    console.log(`  ${acct} ${adj.fullName}: ledger R${adj.beforeBalance.toFixed(2)} + adj R${adj.adjustmentAmount.toFixed(2)} => age R${adj.afterBalance.toFixed(2)} (variance was R${(row?.variance ?? 0).toFixed(2)})`);
}
...
if (!bundle.canImport) {
    ...
    for (const err of bundle.countValidation.errors)
        console.error(`  - ${err}`);
}
...
console.log(`  Expected opening balance adjustments: ${daSilvaFinalImportGate_1.DA_SILVA_FINAL_IMPORT_EXPECTED.openingBalanceAdjustments} (freeze uses ${freezeAdjustments.length} after MAR005 exclusion)`);
console.log(`  Expected age-analysis remaining variance: ${daSilvaFinalImportGate_1.DA_SILVA_FINAL_IMPORT_EXPECTED.ageAnalysisRemainingVariance}`);
console.log(`  Expected merged-family gaps: ${daSilvaFinalImportGate_1.DA_SILVA_FINAL_IMPORT_EXPECTED.mergedFamilyLedgerGaps}`);
