...
function scoreKeeper(learnerId, plans, ledgerCount) {
    const hasPlan = Array.isArray(plans[learnerId]) && plans[learnerId].length > 0;
...
async function main() {
    ...
    const ledgerCountByLearner = new Map();
    ...
    for (const action of actions) {
        ...
        for (const removeId of action.removeIds) {
            ...
            if (plans[removeId]?.length) {
                ...
                (0, learnerBillingPlanStore_1.upsertLearnerBillingPlan)(schoolId, action.keepId, merged);
                (0, learnerBillingPlanStore_1.removeSchoolBillingPlans)(schoolId, [removeId]);
            }
    ...
    const remappedLedger = (0, billingLedgerStore_1.readSchoolLedger)(schoolId).map((entry) => {
        const removeAction = actions.find((a) => a.removeIds.includes(String(entry.learnerId || "")));
        if (!removeAction)
            return entry;
        return { ...entry, learnerId: removeAction.keepId };
    });
    (0, billingLedgerStore_1.writeSchoolLedger)(schoolId, remappedLedger);
    const backfilled = (0, billingLedgerStore_1.backfillLedgerLearnerIds)(schoolId, accountToLearnerId);
