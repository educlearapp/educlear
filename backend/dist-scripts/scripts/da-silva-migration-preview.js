...
const daSilvaVarianceClassification_1 = require("../src/services/daSilvaMigration/daSilvaVarianceClassification");
function isMissingAccountName(fullName) {
...
const classificationCounts = {
    activeAgeAnalysisMismatch: classifiedVariances.filter((v) => v.varianceGroup === "activeAgeAnalysisMismatch").length,
    mergedFamilyLedgerGap: classifiedVariances.filter((v) => v.varianceGroup === "mergedFamilyLedgerGap").length,
    zeroBalanceHistoricalLedgerOnly: classifiedVariances.filter((v) => v.varianceGroup === "zeroBalanceHistoricalLedgerOnly").length,
...
console.log(`1. Active age-analysis balance mismatches: ${classificationCounts.activeAgeAnalysisMismatch}`);
console.log(`2. Merged-family ledger gaps (age analysis authoritative, no adjustment): ${classificationCounts.mergedFamilyLedgerGap}`);
console.log(`3. Zero-balance historical ledger-only accounts: ${classificationCounts.zeroBalanceHistoricalLedgerOnly}`);
...
console.log(`Age-analysis remaining variance (accounts with Kid-e-Sys balance > 0): ${ob.summary.ageAnalysisRemainingVarianceCount} (must be 0)`);
if (ob.adjustments.length) {
    console.log("\nSample adjustments — before / after (first 5):");
    for (const adj of ob.adjustments.slice(0, 5)) {
        console.log(`  ${adj.accountNo} ${adj.fullName}: ledger R${adj.beforeBalance.toFixed(2)} + adj R${adj.adjustmentAmount.toFixed(2)} => age R${adj.afterBalance.toFixed(2)}`);
    }
...
const reportPath = path_1.default.join(__dirname, "..", "variance-report.json");
fs_1.default.writeFileSync(reportPath, JSON.stringify(varianceReport, null, 2), "utf8");
