...
exports.mergeFamilyAccounts = mergeFamilyAccounts;
exports.unmergeLearnerFromFamily = unmergeLearnerFromFamily;
...
const FORBIDDEN_LEARNER_IDENTITY_FIELDS = ["admissionNo", "idNumber"];
function assertLearnerUpdateFieldsAllowed(data) {
    for (const field of FORBIDDEN_LEARNER_IDENTITY_FIELDS) {
        if (Object.prototype.hasOwnProperty.call(data, field)) {
            throw new Error(`Family account operations must not modify learner.${field}`);
        }
    }
}
async function snapshotLearnerIdentities(schoolId, learnerIds) {
...
async function verifyLearnerIdentitiesUnchanged(schoolId, before) {
    if (!before.length)
        return;
    const after = await snapshotLearnerIdentities(schoolId, before.map((l) => l.id));
    const afterById = new Map(after.map((l) => [l.id, l]));
    for (const snap of before) {
        const current = afterById.get(snap.id);
        if (!current)
            continue;
        if (snap.admissionNo !== current.admissionNo ||
            snap.idNumber !== current.idNumber) {
            throw new Error("Learner admission number or ID number changed during family account operation (not allowed)");
        }
    }
}
async function createFamilyAccountRef(schoolId, surname) {
    const prefix = (0, learnerIdentity_1.getSurnamePrefix)(surname);
    const existingCount = await prisma_1.prisma.familyAccount.count({
        where: {
            schoolId,
...
async function loadLearner(schoolId, learnerId) {
    return prisma_1.prisma.learner.findFirst({
...
async function findFamilyAccountById(schoolId, familyAccountId) {
    const id = String(familyAccountId || "").trim();
    if (!id)
        return null;
    return prisma_1.prisma.familyAccount.findFirst({
        where: { id, schoolId },
        select: { id: true, accountRef: true, familyName: true },
    });
}
async function findFamilyAccountByRef(schoolId, accountRef) {
...
function ledgerHasAccountRefOnly(schoolId, accountRef) {
    const ref = String(accountRef || "").trim();
    if (!ref)
        return false;
...
async function resolveMergeFamilyAccount(schoolId, side, opts) {
    ...
    if (accountRef) {
        ...
        if (ledgerHasAccountRefOnly(schoolId, accountRef)) {
            console.warn(`[family-accounts] ${label} validation: accountRef in ledger only, no FamilyAccount row`, { schoolId, accountRef });
            throw new Error(`${label} family account not found (account ref exists in billing ledger only — link learners to a family account first)`);
        }
    ...
    if (learnerId) {
        ...
        const learnerRef = (0, learnerIdentity_1.resolveLearnerAccountNo)({
        ...
        if (learnerRef && learnerRef !== "-") {
            ...
            if (ledgerHasAccountRefOnly(schoolId, learnerRef)) {
                console.warn(`[family-accounts] ${label} validation: learner account ref in ledger only`, { schoolId, learnerId, accountRef: learnerRef });
                throw new Error(`${label} family account not found (account ref exists in billing ledger only — link learners to a family account first)`);
            }
        }
...
async function mergeFamilyAccounts(opts) {
    ...
    if (!hasSource || !hasTarget) {
        ...
        throw new Error("Source and target are required (familyAccountId, accountRef, or learnerId for each side)");
    }
    ...
    const sourceLearners = await prisma_1.prisma.learner.findMany({
        where: { schoolId, familyAccountId: sourceFamilyId },
        select: { id: true, admissionNo: true, idNumber: true },
    });
    const sourceLearnerIds = sourceLearners.map((l) => l.id);
    const identityBefore = sourceLearners.map((l) => ({
        id: l.id,
    ...
    const ledgerResult = (0, billingLedgerStore_1.reassignLedgerAccountRefs)(schoolId, {
    ...
    const audit = (0, familyAccountAuditStore_1.appendFamilyAccountAudit)({
        schoolId,
        action: "merge",
...
async function unmergeLearnerFromFamily(opts) {
    const schoolId = String(opts.schoolId || "").trim();
    const learnerId = String(opts.learnerId || "").trim();
    ...
    if (!sourceFamilyId)
        throw new Error("Learner is not linked to a family account");
    const sourceAccount = await prisma_1.prisma.familyAccount.findFirst({
    ...
    let targetFamilyId = sourceFamilyId;
    let targetAccountRef = oldAccountRef;
    let newFamilyAccount = null;
    const familyMembersBefore = await prisma_1.prisma.learner.findMany({
        where: { schoolId, familyAccountId: sourceFamilyId },
        select: { id: true },
    });
    ...
    if (createNewAccount) {
        ...
        newFamilyAccount = await prisma_1.prisma.familyAccount.create({
            data: {
                schoolId,
                accountRef,
        ...
        targetFamilyId = null;
        targetAccountRef = (0, learnerIdentity_1.resolveLearnerAccountNo)({
            familyAccount: null,
            accountNo: learner.admissionNo,
        });
    }
    ...
    let ledgerMovedEntryIds = [];
    let ledgerSplitEntryIds = [];
    let balanceBefore = null;
    let balanceAfter = null;
    if (targetAccountRef && targetAccountRef !== oldAccountRef) {
        const ledgerResult = (0, billingLedgerStore_1.unmergeLearnerLedger)(schoolId, {
