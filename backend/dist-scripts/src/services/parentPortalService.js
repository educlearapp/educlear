...
exports.phoneOrClause = phoneOrClause;
exports.findParentByCredentials = findParentByCredentials;
exports.normalizeTeacherEmail = normalizeTeacherEmail;
exports.resolveClassroomForLearner = resolveClassroomForLearner;
exports.debugParentThreadResolution = debugParentThreadResolution;
exports.logParentThreadResolution = logParentThreadResolution;
exports.repairAllParentTeacherThreads = repairAllParentTeacherThreads;
exports.syncParentThreadsForClassroom = syncParentThreadsForClassroom;
exports.getOrCreateThread = getOrCreateThread;
exports.createParentNotification = createParentNotification;
exports.notifyParentsForLearner = notifyParentsForLearner;
exports.notifyParentsInvoiceRun = notifyParentsInvoiceRun;
exports.runMigrationParentOnboarding = runMigrationParentOnboarding;
exports.mapThreadMessage = mapThreadMessage;
const prisma_1 = require("../prisma");
const staffJwt_1 = require("../utils/staffJwt");
const communicationEngine_1 = require("../communication/communicationEngine");
const communicationTemplates_1 = require("../communication/communicationTemplates");
...
function phoneOrClause(rawCellNo, idNumber) {
...
function normalizeTeacherEmail(email) {
    return (0, staffJwt_1.normalizeStaffEmail)(email);
}
async function resolveClassroomForLearner(schoolId, learner) {
    const rawClassName = String(learner.className || "").trim();
    if (!rawClassName)
        return null;
    const normalized = (0, classroomNormalization_1.normalizeClassroomInput)(rawClassName, learner.grade != null ? String(learner.grade) : undefined);
    ...
    for (const c of existingClassrooms) {
        const key = (0, classroomNormalization_1.normalizeClassroomInput)(c.name).matchKey;
        if (key && normalized.matchKey && key === normalized.matchKey) {
            if (c.name !== canonicalName) {
                return prisma_1.prisma.classroom.update({
                    where: { id: c.id },
        ...
        if (c.name === canonicalName || c.name === rawClassName)
            return c;
    }
    return prisma_1.prisma.classroom.create({
        data: {
            schoolId,
            name: canonicalName,
            teacherName: "",
...
async function debugParentThreadResolution(opts) {
    ...
    let thread = null;
    if (opts.parentId) {
        thread = await prisma_1.prisma.parentTeacherThread.findUnique({
            where: {
        ...
        thread = await prisma_1.prisma.parentTeacherThread.findFirst({
...
function logParentThreadResolution(label, debug, loggedInTeacherEmail) {
...
async function syncThreadTeacherFromClassroom(threadId, classroom) {
    ...
    return prisma_1.prisma.parentTeacherThread.update({
        where: { id: threadId },
        data: {
...
async function repairAllParentTeacherThreads(opts) {
    ...
    for (const thread of threads) {
        ...
        if (thread.classroomId !== classroomId ||
...
async function syncParentThreadsForClassroom(schoolId, classroomId) {
    const classroom = await prisma_1.prisma.classroom.findFirst({
        where: { id: classroomId, schoolId },
    });
    if (!classroom)
        return { updated: 0 };
    const variants = (0, classroomNormalization_1.classroomNameVariants)((0, classroomNormalization_1.normalizeClassroomInput)(classroom.name));
    ...
    const teacherName = String(classroom.teacherName || "").trim() || "Class Teacher";
    const teacherEmail = normalizeTeacherEmail(classroom.teacherEmail);
    const result = await prisma_1.prisma.parentTeacherThread.updateMany({
...
async function getOrCreateThread(opts) {
    ...
    const teacherName = String(classroom?.teacherName || "").trim() || "Class Teacher";
    const teacherEmail = normalizeTeacherEmail(classroom?.teacherEmail || "");
    const existing = await prisma_1.prisma.parentTeacherThread.findUnique({
        where: {
            schoolId_parentId_learnerId: {
                schoolId: opts.schoolId,
                parentId: opts.parentId,
    ...
    if (existing) {
        const needsSync = existing.classroomId !== (classroom?.id ?? null) ||
    ...
    const thread = await prisma_1.prisma.parentTeacherThread.create({
        data: {
            schoolId: opts.schoolId,
            parentId: opts.parentId,
            learnerId: opts.learnerId,
...
function templateKeyForParentType(t) {
    switch (t) {
        case "INVOICE_READY":
...
async function createParentNotification(opts) {
    ...
    const templateVariables = {
        schoolName,
        portalUrl: communicationEngine_1.portalUrlDefault,
    ...
    const { parentNotification } = await (0, communicationEngine_1.submitParentInAppNotification)({
        schoolId: opts.schoolId,
        parentId: opts.parentId,
...
async function notifyParentsInvoiceRun(opts) {
    await (0, communicationEngine_1.ensureDefaultCommunicationTemplates)(prisma_1.prisma);
    ...
    const schoolName = school?.name || "School";
    const portalUrl = communicationEngine_1.portalUrlDefault;
    const templateVars = {
        month: monthLabel,
        runId: opts.runId,
        schoolName,
    ...
    for (const p of parents) {
        if (seen.has(p.id))
            continue;
        seen.add(p.id);
        await (0, communicationEngine_1.submitParentInAppNotification)({
            schoolId: opts.schoolId,
            parentId: p.id,
        ...
        await (0, communicationEngine_1.queueBillingFollowupsForParent)({
            schoolId: opts.schoolId,
            parentId: p.id,
            monthLabel,
            runId: opts.runId,
...
async function runMigrationParentOnboarding(schoolId) {
    await (0, communicationEngine_1.ensureDefaultCommunicationTemplates)(prisma_1.prisma);
    const school = await prisma_1.prisma.school.findUnique({
        where: { id: schoolId },
    ...
    const portalUrl = communicationEngine_1.portalUrlDefault;
    let invited = 0;
    const campaign = await prisma_1.prisma.communicationCampaign.create({
        data: {
            schoolId,
    ...
    for (const parent of parents) {
        const hasContact = Boolean(String(parent.email || "").trim()) || Boolean(String(parent.cellNo || "").trim());
        if (!hasContact)
            continue;
        ...
        const rendered = await (0, communicationTemplates_1.resolveRenderedMessage)(prisma_1.prisma, schoolId, "onboarding_welcome", templateVars);
        ...
        await (0, communicationEngine_1.queueParentOutreachChannels)({
            schoolId,
            parentId: parent.id,
            channels,
            subject,
