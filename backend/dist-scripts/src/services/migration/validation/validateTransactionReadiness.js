...
exports.validateLearnerStatusRows = validateLearnerStatusRows;
exports.validateTransactionReadinessRows = validateTransactionReadinessRows;
exports.validateTransactionReadiness = validateTransactionReadiness;
const computeTransactionReadiness_1 = require("../core/computeTransactionReadiness");
const transactionEligibility_1 = require("../core/transactionEligibility");
...
function validateTransactionReadinessRows(input) {
    ...
    if (!isTransactionFile)
        return issues;
    const hasAmountMapping = [...AMOUNT_FIELDS].some((f) => targetToSource.has(f));
    const hasDateMapping = targetToSource.has("transactionDate");
    rows.forEach((row, idx) => {
        const rowNumber = idx + 1;
        const { entry, matched } = (0, computeTransactionReadiness_1.resolveLearnerForRow)(row, targetToSource, learnerIndex);
        const learnerStatus = entry?.status ?? null;
        ...
        if (hasAmountMapping) {
            ...
            if (!anyAmount) {
                ...
                issues.push(issue({
                    ...
                    value: cellString(getMappedValue(row, targetToSource, "amount")),
                }));
            }
        }
        const hasMatchField = targetToSource.has("accountNumber") ||
        ...
        if (learnerStatus && (0, MigrationLearnerStatus_1.requiresReviewBeforeApply)(learnerStatus)) {
            issues.push(issue({
                fileId: preview.fileId,
                filename: preview.filename,
                rowNumber,
        ...
        const historicalOnly = (0, transactionEligibility_1.shouldTransactionBeHistoricalOnly)({
            learnerStatus,
        ...
        const bucket = (0, transactionEligibility_1.classifyTransactionReadiness)({
            ...
            amountValid: !hasAmountMapping || [...AMOUNT_FIELDS].some((f) => {
                if (!targetToSource.has(f))
                    return false;
                const raw = getMappedValue(row, targetToSource, f);
                return cellString(raw).length > 0 && isNumericValue(raw);
            }),
            datePresent,
        });
        if (learnerStatus === "HISTORICAL" &&
            !historicalOnly &&
            ...
            issues.push(issue({
                ...
                category: preview.category,
                field: "status",
                message: "Transaction mapped to HISTORICAL learner may affect new billing — will be treated as historical-only",
                value: learnerStatus,
            }));
        }
        if (learnerStatus &&
            ...
            const eligible = entry
                ? (0, transactionEligibility_1.isLearnerEligibleForNewBilling)({
                    status: entry.status,
                    grade: entry.grade,
                    classroom: entry.classroom,
            ...
            if (!eligible && bucket !== "historicalOnly") {
                issues.push(issue({
                    ...
                    category: preview.category,
                    field: "status",
                    message: "Active billing transaction is linked to inactive (historical/unenrolled) learner",
                    value: learnerStatus,
                }));
            }
        }
...
function validateTransactionReadiness(input) {
    const mappingsByFile = new Map(input.mappings.map((m) => [m.fileId, m]));
    const learnerIndex = (0, computeTransactionReadiness_1.buildMigrationLearnerMatchIndex)(input.previews, mappingsByFile, input.rowsByFileId);
    const issues = [];
    for (const preview of input.previews) {
        ...
        issues.push(...validateLearnerStatusRows({ preview, fileMappings, rows }), ...validateTransactionReadinessRows({
            preview,
            fileMappings,
            rows,
            learnerIndex,
