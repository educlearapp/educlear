...
exports.buildKidESysMigrationReadiness = buildKidESysMigrationReadiness;
...
function learnerDisplayName(row, targetToSource) {
...
function buildDetailLine(def, fileCount, rowCount, entityCount) {
    const parts = [];
    if (fileCount > 0) {
...
function buildCategoryRows(previews, uploadedFiles) {
    const categoriesPresent = new Set();
    for (const f of uploadedFiles) {
        if (f.category && f.category !== "unknown")
...
function findDuplicateKeys(keys) {
    ...
    for (const [key, meta] of byKey) {
        if (meta.count > 1)
            dupes.push({ key, count: meta.count, label: meta.label });
    }
    return dupes;
}
async function loadRowsForPreview(preview, filePaths, fullFileChecks) {
    if (!fullFileChecks) {
    ...
    const full = await (0, readMigrationFileRows_1.readMigrationFileRows)(file);
    return full.rows;
}
async function runCrossValidation(input) {
    ...
    const warnings = [];
    const scope = fullFileChecks ? "full_file" : "preview_sample";
            ...
            for (const row of rows) {
                if (hasAccountMapping) {
                    ...
                    if (!acc) {
                        ...
                        if (accountSamples.length < 5) {
                            accountSamples.push(cellString(getMappedValue(row, targetToSource, "reference")) || "(no account ref)");
                        }
                    }
                }
                for (const field of amountFields) {
                    ...
                    if (s && !isNumericValue(raw)) {
                        invalidAmount += 1;
                        if (amountSamples.length < 5)
                            amountSamples.push(s);
                        break;
                    }
                }
            }
 ...
 */
async function buildKidESysMigrationReadiness(input) {
    ...
    const readyForMigration = learners?.status === "found" && parents?.status === "found";
    ...
    const { warnings: crossValidationWarnings, scope: crossValidationScope } = await runCrossValidation(input);
    return {
        systemId: "kideesys",
        readyForMigration,
        proceedStatus,
