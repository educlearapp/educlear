...
exports.resolveLedgerPostingType = resolveLedgerPostingType;
exports.formatLedgerDuplicateKey = formatLedgerDuplicateKey;
exports.classifyLedgerTransaction = classifyLedgerTransaction;
...
function parseAmountFromMapped(mapped) {
    ...
    const c = pick(credit);
    if (d != null && !c)
        return { valid: true, amount: d };
    if (c != null && !d)
        return { valid: true, amount: c };
    if (d != null && c != null)
        return { valid: true, amount: Math.max(d, c) };
    if (!amountRaw && !debit && !credit)
        return { valid: false, amount: 0 };
    return { valid: false, amount: 0 };
}
...
function resolveLedgerPostingType(rawType) {
    ...
    if (t === "dr" || t === "debit note")
        return "invoice";
    if (t === "cr" || t === "credit note")
        return "payment";
    return "unknown";
}
function ledgerDuplicateKey(parts) {
    ...
    return {
        ...
        amount: (0, billingLedgerStore_1.normaliseAmount)(parts.amount),
        postingType: parts.postingType,
    };
}
function duplicateKeyString(key) {
    return `tx:${key.accountRef}|${key.date}|${key.reference}|${key.amount}|${key.postingType}`;
}
function formatLedgerDuplicateKey(key) {
    return duplicateKeyString(key);
}
function classifyLedgerTransaction(input) {
    ...
    const learnerStatus = learnerEntry?.status ?? null;
    const accountStatus = cleanString(input.accountStatus) || learnerEntry?.accountStatus || "";
    const accountClosed = Boolean(input.accountClosed) || (0, MigrationLearnerStatus_1.isClosedOrInactiveAccountStatus)(accountStatus);
    const readinessBucket = (0, transactionEligibility_1.classifyTransactionReadiness)({
        learnerStatus,
    ...
    if (readinessBucket === "blocked") {
        const reason = learnerStatus && learnerStatus !== "ACTIVE"
