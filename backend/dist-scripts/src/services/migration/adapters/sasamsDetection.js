...
exports.evaluateSASAMSDetection = evaluateSASAMSDetection;
exports.detectSASAMSExports = detectSASAMSExports;
const sasamsMetadata_1 = require("./sasamsMetadata");
const sasamsNormalization_1 = require("./sasamsNormalization");
...
function scoreFilenameSignals(files) {
    ...
    for (const file of files) {
        ...
        if (h.includes("learnerregister") || (h.includes("learner") && h.includes("register")))
            score += 2;
        if (h.includes("classlist") || (h.includes("class") && h.includes("list")))
            score += 2;
        if (h.includes("educator"))
            score += 1;
        if (h.includes("parent") || h.includes("guardian"))
            score += 1;
        if (h.includes("contact") && (h.includes("list") || h.includes("parent")))
            score += 2;
        if (h.includes("register") && !h.includes("registerclass"))
            score += 1;
    }
...
function columnMatchesAny(column, keys) {
    ...
    if (keys.some((k) => compact === k || compact.includes(k)))
        return true;
    const normalized = (0, sasamsNormalization_1.normalizeSASAMSColumn)(column);
...
function countHeaderGroups(columns) {
    ...
    if (columns.some((c) => columnMatchesAny(c, parentKeys)))
        groups += 1;
    if (columns.some((c) => columnMatchesAny(c, adminKeys)))
        groups += 1;
    return groups;
}
function evaluateSASAMSDetection(input) {
    ...
    const filenamePass = filenameScore >= rules.minFilenameScore &&
...
function detectSASAMSExports(filenames, columns) {
