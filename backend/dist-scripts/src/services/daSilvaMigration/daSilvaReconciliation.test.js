...
const bundle = (0, daSilvaMigrationService_1.buildDaSilvaBundleFromDesktopLayout)("test-school", "test-project", desktopRoot);
...
if (Math.abs(ada004.variance) > 0.01) {
    throw new Error(`ADA004 expected zero variance, got age=${ada004.ageAnalysisBalance} ledger=${ada004.ledgerBalanceFromImport} variance=${ada004.variance}`);
}
...
if (Math.abs(sil007.variance) > 0.01) {
    throw new Error(`SIL007 merged family should reconcile to age analysis, variance=${sil007.variance}`);
}
...
for (const adj of bundle.openingBalance.adjustments) {
    ...
    if (Math.abs(projected - adj.afterBalance) > 0.01) {
        throw new Error(`${adj.accountNo} adjustment does not reach age analysis: before=${adj.beforeBalance} adj=${adj.adjustmentAmount} after=${adj.afterBalance}`);
    }
}
...
if (bundle.openingBalance.summary.ageAnalysisRemainingVarianceCount !== 0) {
    throw new Error(`Expected 0 age-analysis variances after opening adjustments, got ${bundle.openingBalance.summary.ageAnalysisRemainingVarianceCount}`);
}
...
console.log(`Age-analysis variance after adjustments: ${bundle.openingBalance.summary.ageAnalysisRemainingVarianceCount}`);
