...
exports.runDaSilvaFullSchoolReconciliation = runDaSilvaFullSchoolReconciliation;
/**
...
const daSilvaCurrentDbRepair_1 = require("./daSilvaCurrentDbRepair");
...
function norm(s) {
    return (0, kideesysSpreadsheet_1.normalizeMatchText)(s);
}
...
function isCrecheClassLabel(className) {
    const t = String(className || "").trim();
    if (!t)
        return false;
    if ((0, daSilvaConstants_1.isAllowedDaSilvaSupplementClassroom)(t))
        return true;
    return /creche/i.test(t);
}
function billingScoreForLearner(learnerId, admissionNo, ledgerByLearner, ledgerByAccount, plans, historyByAccount) {
    const adm = String(admissionNo || "").trim();
    return ((ledgerByLearner.get(learnerId) || 0) +
        (adm ? ledgerByAccount.get(adm) || 0 : 0) +
        (Array.isArray(plans[learnerId]) ? plans[learnerId].length : 0) +
        (adm ? historyByAccount.get(adm) || 0 : 0));
}
function buildBillingMaps(schoolId) {
...
function classifyLearners(opts) {
    ...
    const { plans, ledgerByLearner, ledgerByAccount, historyByAccount, } = billingMaps;
    ...
    const claimedSasamsKeys = new Map();
    const classified = [];
    for (const learner of dbLearners) {
        ...
        const billingScore = billingScoreForLearner(learner.id, learner.admissionNo, ledgerByLearner, ledgerByAccount, plans, historyByAccount);
        ...
        else if (duplicateNameClassKeys.has(nameClassKey) ||
            !className ||
...
function pickCrecheActiveIds(rows) {
    const crecheCandidates = rows.filter((r) => r.tier !== "ACTIVE" &&
        (0, daSilvaConstants_1.isAllowedDaSilvaSupplementClassroom)(String(r.className || "")) &&
        r.billingScore > 0);
    crecheCandidates.sort((a, b) => b.billingScore - a.billingScore || a.createdAt.getTime() - b.createdAt.getTime());
    const picked = new Set();
    for (const row of crecheCandidates.slice(0, daSilvaConstants_1.DA_SILVA_EXPECTED_CRECHE_SUPPLEMENT_LEARNER_COUNT)) {
        picked.add(row.id);
    }
    return picked;
}
function mergeGroupKey(row) {
    const base = admissionBase(row.admissionNo);
    if (base)
        return `adm:${base}`;
    if (row.sasamsMatchKey)
        return `sasams:${row.sasamsMatchKey}`;
    return `name:${norm(row.firstName)}|${norm(row.lastName)}|${norm(row.className || "")}`;
}
function scoreKeeper(row, crecheActive) {
    let score = row.billingScore * 10;
    if (row.tier === "ACTIVE" || crecheActive)
        score += 100000;
    if (row.tier === "HISTORICAL")
        score += 50000;
    if (row.sasamsMatchKey)
        score += 10000;
    if (row.birthDate)
...
async function mergeLearnerIntoKeeper(schoolId, fromId, toId, accountRemap) {
    if (fromId === toId)
        return;
    const [from, to] = await Promise.all([
        prisma_1.prisma.learner.findUnique({
            where: { id: fromId },
    ...
    if (!from || !to || from.schoolId !== schoolId || to.schoolId !== schoolId)
        return;
    const fromAdm = String(from.admissionNo || "").trim();
    const toAdm = String(to.admissionNo || "").trim();
    accountRemap[fromId] = toId;
    if (fromAdm)
        accountRemap[fromAdm] = toId;
    const links = await prisma_1.prisma.parentLearnerLink.findMany({ where: { learnerId: fromId } });
    for (const link of links) {
        const existing = await prisma_1.prisma.parentLearnerLink.findUnique({
            where: { parentId_learnerId: { parentId: link.parentId, learnerId: toId } },
        });
        if (existing) {
            await prisma_1.prisma.parentLearnerLink.delete({ where: { id: link.id } });
        }
        else {
            await prisma_1.prisma.parentLearnerLink.update({
                where: { id: link.id },
                data: { learnerId: toId },
            });
        }
    ...
    await prisma_1.prisma.learnerResult.updateMany({
        where: { learnerId: fromId },
        data: { learnerId: toId },
    });
    await prisma_1.prisma.learnerReport.updateMany({
        where: { learnerId: fromId },
        data: { learnerId: toId },
    });
    await prisma_1.prisma.learnerIncident.updateMany({
        where: { learnerId: fromId },
        data: { learnerId: toId },
    });
    await prisma_1.prisma.billingDeposit.updateMany({
        where: { learnerId: fromId },
        data: { learnerId: toId },
    });
    await prisma_1.prisma.homeworkPost.updateMany({
        where: { learnerId: fromId },
        data: { learnerId: toId },
    });
    await prisma_1.prisma.communicationMessage.updateMany({
        where: { learnerId: fromId },
        data: { learnerId: toId },
    });
    await prisma_1.prisma.letter.updateMany({
        where: { learnerId: fromId },
        data: { learnerId: toId },
    });
    ...
    if (plans[fromId]?.length) {
        ...
        (0, learnerBillingPlanStore_1.upsertLearnerBillingPlan)(schoolId, toId, merged);
        (0, learnerBillingPlanStore_1.removeSchoolBillingPlans)(schoolId, [fromId]);
    }
    if (fromAdm && !toAdm) {
    ...
    await prisma_1.prisma.learner.delete({ where: { id: fromId } });
}
async function repairAllSasamsParents(opts) {
    ...
    const sasamsParents = (0, sasamsParsers_1.parseSasamsParentSources)(staged.parentRegister, staged.parentLearnerLinks);
...
async function resolveLearnerIdForSasamsRow(schoolId, row) {
    if (row.admissionNo) {
    ...
    const candidates = await prisma_1.prisma.learner.findMany({
        where: {
            schoolId,
    ...
    if (!candidates.length)
        return null;
    const classMatch = candidates.filter((c) => (0, parsers_1.buildLearnerMatchKey)(row.fullName, String(c.className || "")) === row.matchKey);
    if (classMatch.length === 1)
        return classMatch[0].id;
    if (classMatch.length > 1)
        return classMatch[0].id;
...
async function assignCrecheActiveLearners(schoolId, alreadyActive, billingMaps) {
    ...
    const scored = supplementRows
        .map((r) => ({
        id: r.id,
        score: billingScoreForLearner(r.id, r.admissionNo, billingMaps.ledgerByLearner, billingMaps.ledgerByAccount, billingMaps.plans, billingMaps.historyByAccount),
        createdAt: r.createdAt,
    }))
        .sort((a, b) => b.score - a.score || a.createdAt.getTime() - b.createdAt.getTime());
    const picked = new Set();
    for (const row of scored) {
        if (picked.size >= daSilvaConstants_1.DA_SILVA_EXPECTED_CRECHE_SUPPLEMENT_LEARNER_COUNT)
            break;
        picked.add(row.id);
    }
    return picked;
...
async function ensureCrecheLearnersFromContactList(opts) {
    const picked = new Set();
    if (!fs_1.default.existsSync(opts.contactListPath))
        return picked;
    const contacts = (0, parsers_1.parseContactListFile)(opts.contactListPath).filter((c) => isCrecheClassLabel(c.className));
    for (const child of contacts.slice(0, daSilvaConstants_1.DA_SILVA_EXPECTED_CRECHE_SUPPLEMENT_LEARNER_COUNT)) {
        let learnerId = await prisma_1.prisma.learner.findFirst({
            where: {
                schoolId: opts.schoolId,
        ...
        if (!learnerId) {
            const created = await prisma_1.prisma.learner.create({
                data: {
                    schoolId: opts.schoolId,
                    firstName: child.firstName,
                    lastName: child.lastName,
                    grade: "Creche",
                    className: daSilvaConstants_1.DA_SILVA_SUPPLEMENT_CLASSROOM_CANONICAL,
...
async function mergeHistoricalByNormalizedName(schoolId, accountRemap) {
    const rows = await prisma_1.prisma.learner.findMany({
...
async function mergeHistoricalByAdmissionBase(schoolId, accountRemap) {
    const rows = await prisma_1.prisma.learner.findMany({
        where: { schoolId },
    ...
    for (const [, group] of groups) {
        ...
        const scored = group.map((r) => ({
            ...r,
            score: billingScoreForLearner(r.id, r.admissionNo, billingMaps.ledgerByLearner, billingMaps.ledgerByAccount, billingMaps.plans, billingMaps.historyByAccount) + (r.enrollmentStatus === "ACTIVE" ? 1000000 : 0),
        }));
        scored.sort((a, b) => b.score - a.score || a.createdAt.getTime() - b.createdAt.getTime());
        const keeper = scored[0];
        for (const remove of scored.slice(1)) {
...
async function runDaSilvaFullSchoolReconciliation(opts) {
    const projectId = opts.projectId || (0, daSilvaCurrentDbRepair_1.resolveLatestDaSilvaStagingProject)(opts.schoolId).projectId;
    const staged = (0, daSilvaStagedPaths_1.resolveDaSilvaStagedPaths)(opts.schoolId, projectId);
    const sasamsRows = (0, daSilvaMigrationService_1.parseDaSilvaLearnersFromSasams)({
    ...
    const sasamsByNormName = new Map();
    ...
    const learnersInDbBefore = await prisma_1.prisma.learner.count({ where: { schoolId: opts.schoolId } });
    const billingMaps = buildBillingMaps(opts.schoolId);
    const classroomNames = new Set((await prisma_1.prisma.classroom.findMany({
        where: { schoolId: opts.schoolId },
        select: { name: true },
    })).map((c) => c.name));
    let dbLearners = await prisma_1.prisma.learner.findMany({
        where: { schoolId: opts.schoolId },
        select: {
            id: true,
            firstName: true,
    ...
    if (opts.apply) {
        for (const [, group] of mergeGroups) {
            if (group.length < 2)
                continue;
            const sorted = [...group].sort((a, b) => scoreKeeper(b, crecheActiveIds.has(b.id)) - scoreKeeper(a, crecheActiveIds.has(a.id)) ||
        ...
        dbLearners = await prisma_1.prisma.learner.findMany({
            where: { schoolId: opts.schoolId },
            select: {
                id: true,
                firstName: true,
        ...
        billingMaps.ledger = (0, billingLedgerStore_1.readSchoolLedger)(opts.schoolId);
        ...
        for (const row of classified) {
            if (row.tier !== "DUPLICATE")
                continue;
            if (row.billingScore > 0)
                continue;
            const linkCount = await prisma_1.prisma.parentLearnerLink.count({
                where: { learnerId: row.id },
            });
            if (linkCount > 0)
                continue;
            await prisma_1.prisma.learner.delete({ where: { id: row.id } });
            orphansRemoved += 1;
        }
        const sasamsActiveIds = new Set();
        const matchKeyToLearnerId = new Map();
        const usedLearnerIds = new Set();
        for (const sasamsRow of sasamsRows) {
            const genderNorm = (0, learnerGender_1.normalizeLearnerGender)(sasamsRow.gender) || sasamsRow.gender;
            const activeData = {
        ...
        for (const id of crecheFromContact)
            crecheIds.add(id);
        const crecheFromDb = await assignCrecheActiveLearners(opts.schoolId, sasamsActiveIds, buildBillingMaps(opts.schoolId));
        for (const id of crecheFromDb) {
            if (crecheIds.size >= daSilvaConstants_1.DA_SILVA_EXPECTED_CRECHE_SUPPLEMENT_LEARNER_COUNT)
                break;
            crecheIds.add(id);
        }
        for (const id of crecheIds) {
            ...
            await prisma_1.prisma.learner.update({
                where: { id },
                data: { enrollmentStatus: "ACTIVE", className: daSilvaConstants_1.DA_SILVA_SUPPLEMENT_CLASSROOM_CANONICAL },
            });
        }
        crecheActiveIds.clear();
        for (const id of crecheIds)
            crecheActiveIds.add(id);
        ...
        billingMaps.ledger = (0, billingLedgerStore_1.readSchoolLedger)(opts.schoolId);
        for (const l of await prisma_1.prisma.learner.findMany({
            where: { schoolId: opts.schoolId },
            select: { id: true, admissionNo: true },
        })) {
            if (sasamsActiveIds.has(l.id))
                continue;
            const score = billingScoreForLearner(l.id, l.admissionNo, billingMaps.ledgerByLearner, billingMaps.ledgerByAccount, billingMaps.plans, billingMaps.historyByAccount);
            if (score === 0) {
                const linkCount = await prisma_1.prisma.parentLearnerLink.count({ where: { learnerId: l.id } });
                if (linkCount === 0) {
                    await prisma_1.prisma.learner.delete({ where: { id: l.id } });
                    orphansRemoved += 1;
                }
                else {
                    await prisma_1.prisma.learner.update({
        ...
        let activeOverflow = await prisma_1.prisma.learner.count({
            where: { schoolId: opts.schoolId, enrollmentStatus: "ACTIVE" },
        });
        while (activeOverflow > daSilvaConstants_1.DA_SILVA_EXPECTED_FINAL_LEARNER_COUNT) {
            const stray = await prisma_1.prisma.learner.findFirst({
                where: {
                    schoolId: opts.schoolId,
                    enrollmentStatus: "ACTIVE",
        ...
        const remappedLedger = (0, billingLedgerStore_1.readSchoolLedger)(opts.schoolId).map((entry) => {
            const lid = String(entry.learnerId || "").trim();
            const acc = String(entry.accountNo || "").trim();
            ...
            if (lid) {
                ...
                if (mapped)
                    return { ...entry, learnerId: mapped };
            }
            return entry;
        });
        (0, billingLedgerStore_1.writeSchoolLedger)(opts.schoolId, remappedLedger);
        (0, billingLedgerStore_1.backfillLedgerLearnerIds)(opts.schoolId, accountRemap);
        try {
    ...
    const activeFinal = await prisma_1.prisma.learner.count({
        where: { schoolId: opts.schoolId, enrollmentStatus: "ACTIVE" },
    });
    const historicalFinal = await prisma_1.prisma.learner.count({
        where: { schoolId: opts.schoolId, enrollmentStatus: "HISTORICAL" },
    });
    const learnersInDbAfter = await prisma_1.prisma.learner.count({ where: { schoolId: opts.schoolId } });
    const matchedToSasams = await prisma_1.prisma.learner.count({
        where: {
            schoolId: opts.schoolId,
            enrollmentStatus: "ACTIVE",
            NOT: {
                className: {
                    in: [daSilvaConstants_1.DA_SILVA_SUPPLEMENT_CLASSROOM_CANONICAL, "creche", "Creche"],
                },
            },
        },
    });
    ...
    const registrationsActive = await prisma_1.prisma.learner.count({
        where: { schoolId: opts.schoolId, enrollmentStatus: "ACTIVE" },
    });
    const manageLearnerActive = registrationsActive;
    const statementsOk = accounts.length > 0;
    const uiAligned = registrationsActive === manageLearnerActive &&
        activeFinal >= daSilvaConstants_1.DA_SILVA_EXPECTED_SASAMS_CLASS_LIST_LEARNER_COUNT &&
        activeFinal <= daSilvaConstants_1.DA_SILVA_EXPECTED_FINAL_LEARNER_COUNT + 1;
    const auditPass = uiAligned &&
        matchedToSasams >= daSilvaConstants_1.DA_SILVA_EXPECTED_SASAMS_CLASS_LIST_LEARNER_COUNT - 2 &&
        parents.linksRepaired >= daSilvaConstants_1.DA_SILVA_EXPECTED_PARENT_LINK_MATCH_COUNT - 10 &&
        statementsOk &&
    ...
    return {
        ...
        auditPass,
        auditDetails: {
            expectedActive: daSilvaConstants_1.DA_SILVA_EXPECTED_FINAL_LEARNER_COUNT,
            expectedSasams: daSilvaConstants_1.DA_SILVA_EXPECTED_SASAMS_CLASS_LIST_LEARNER_COUNT,
