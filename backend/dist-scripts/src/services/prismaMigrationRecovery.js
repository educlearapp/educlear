...
exports.runPrismaMigrateDeployWithRecovery = runPrismaMigrateDeployWithRecovery;
...
function markMigrationRolledBack(migrationName) {
    ...
    console.log(`[startup] Resolving failed migration (rolled-back): ${migrationName}`);
    const output = (0, child_process_1.execSync)(`npx prisma migrate resolve --rolled-back ${migrationName}`, {
        cwd: backendRoot,
        encoding: "utf-8",
        env: process.env,
...
function resolveFailedMigration(migrationName) {
    try {
        ...
        console.log(`[startup] migrate resolve --rolled-back skipped for ${migrationName} (no failed record or already resolved)`);
...
async function resolveAllFailedMigrationsBeforeDeploy() {
    const failed = await findFailedMigrations();
    if (failed.length === 0) {
        console.log("[startup] No failed migrations in _prisma_migrations");
        return;
    }
    console.log(`[startup] Found ${failed.length} failed migration(s): ${failed.join(", ")}`);
...
function isP3009Error(combined) {
    return combined.includes("P3009");
}
function logDeployFailure(label, error, err) {
...
async function runPrismaMigrateDeployWithRecovery() {
    ...
    try {
        ...
        const shouldRetry = isP3009Error(combined) || stillFailed.length > 0;
        if (shouldRetry) {
            try {
                console.log(`[startup] Migration deploy blocked — resolving ${stillFailed.length || "any"} failed migration(s) and retrying deploy once…`);
                await resolveAllFailedMigrationsBeforeDeploy();
                runMigrateDeploy();
                return;
            }
            catch (retryError) {
                const retryErr = retryError;
                logDeployFailure("prisma migrate deploy failed after resolve", retryError, retryErr);
                return;
            }
        }
        logDeployFailure("prisma migrate deploy failed (continuing startup)", error, err);
    }
}
