...
const requireAuthDiagnostics_1 = require("../middleware/requireAuthDiagnostics");
const prisma_1 = require("../prisma");
const authCredentials_1 = require("../services/authCredentials");
const authDiagnostics_1 = require("../services/authDiagnostics");
...
const ownerProvisioning_1 = require("../utils/ownerProvisioning");
...
const normalizeEmail = authCredentials_1.normalizeAuthEmail;
function signAuthToken(payload) {
    return jsonwebtoken_1.default.sign(payload, JWT_SECRET, { expiresIn: "7d" });
}
function authLog(message, extra) {
    if (extra) {
...
router.post("/login", async (req, res) => {
    ...
    try {
        ...
        for (const user of users) {
            const ok = await (0, authCredentials_1.compareAuthPassword)(password, user.passwordHash);
        ...
        const educlearRole = (0, superAdmin_1.isPlatformSuperAdminEmail)(matched.email)
            ? "superAdmin"
            : undefined;
        const migrationCtx = {
            userId: matched.id,
        ...
        const canAccessMigrationFlag = (0, migrationAccess_1.canAccessMigration)(migrationCtx);
        const permissions = (0, userPermissions_1.permissionsForRole)(matched.role);
        return res.json({
            token,
...
router.get("/diagnostics/:email", requireAuthDiagnostics_1.requireAuthDiagnostics, async (req, res) => {
    const email = normalizeEmail(req.params.email);
    if (!email) {
        return res.status(400).json({ error: "Email required" });
    }
    try {
        const diag = await (0, authDiagnostics_1.buildAuthDiagnostics)(email);
        return res.json({
...
router.post("/register-school", async (req, res) => {
    ...
    const logoUrl = body.logoUrl ? (0, schoolLogo_1.toStoredSchoolLogoUrl)(String(body.logoUrl).trim()) : null;
    ...
    try {
        const existingSchoolByEmail = await prisma_1.prisma.school.findFirst({
        ...
        const schoolByName = await prisma_1.prisma.school.findFirst({
            where: { name: { equals: schoolName, mode: "insensitive" } },
            select: { id: true, name: true, email: true, phone: true, logoUrl: true },
        });
        if (schoolByName) {
            const schoolUsers = await prisma_1.prisma.user.findMany({
                where: { schoolId: schoolByName.id, isActive: true },
                select: { id: true, email: true, fullName: true },
            });
            const scriptOwner = schoolUsers.find((u) => (0, ownerProvisioning_1.normalizeOwnerEmail)(u.email) === email &&
                (0, ownerProvisioning_1.isScriptProvisionedOwner)(u, schoolByName));
            if (scriptOwner) {
                ...
                return res.status(400).json({
                    error: "A school with this name already exists. Please log in with your owner account.",
                });
            }
            ...
            const passwordHash = await (0, authCredentials_1.hashAuthPassword)(password);
                ...
                const user = scriptOwner
                    ? await tx.user.update({
                        ...
                        data: {
                            ...
                            role: (0, userPermissions_1.prismaRoleForAppRole)("Owner"),
                            isActive: true,
                        },
                        select: {
                            id: true,
            ...
            (0, userAccessStore_1.setUserAccessMeta)(result.user.id, {
            ...
            try {
                await (0, schoolEmailService_1.seedSchoolEmailDefaults)(result.school.id);
            }
            catch (seedErr) {
        ...
        if (existingUsers.length) {
            const withSchool = await Promise.all(existingUsers.map(async (u) => {
                const school = await prisma_1.prisma.school.findUnique({
                    where: { id: u.schoolId },
            ...
            const registrationRow = withSchool.find(({ user, school }) => school && (0, ownerProvisioning_1.isRegistrationProvisionedOwner)(user, school));
            if (registrationRow) {
                return res.status(400).json({
                    error: "An account with this email already exists. Please log in.",
                });
