...
function calculatePayroll(payeBase, uifBase) {
...
router.post("/employee", async (req, res) => {
    try {
        const { schoolId, firstName, lastName, email, idNumber, basicSalary, dateOfBirth, taxNumber, uifApplicable, incomeTaxApplicable, physicalAddress, bankName, bankAccountHolder, bankAccountNumber, bankBranchCode, employeeNumber, jobTitle, employeePension, employeeMedicalAid, employerMedicalAid, overtimeHours, overtimeRate, } = req.body;
        const num = (v, fallback = 0) => Number(v === undefined || v === null || v === "" ? fallback : v);
...
router.post("/run", async (req, res) => {
    try {
        ...
        for (const emp of employees) {
            ...
            const grossEarnings = Number((basicSalaryNum + overtimePay + employerMedicalNum).toFixed(2));
            ...
            const otherDeductions = Number((employeePensionNum + employeeMedicalNum).toFixed(2));
            const totalDeductions = Number((paye + uifEmployee + otherDeductions).toFixed(2));
            ...
            await prisma.payslip.create({
                data: {
                    schoolId,
                    payrollRunId: payrollRun.id,
                    payrollRunEmployeeId: (await prisma.payrollRunEmployee.findFirst({
                        where: {
                            payrollRunId: payrollRun.id,
                            employeeId: emp.id,
                        },
...
router.post("/email-payslip", async (req, res) => {
    try {
        ...
        const from = process.env.SMTP_FROM?.trim() || process.env.SMTP_USER?.trim() || "noreply@educlear";
        const subjectParts = [`Payslip`, schoolName];
        if (periodLabel)
            subjectParts.push(periodLabel);
