...
router.get("/presets", (_req, res) => {
    return res.json({ success: true, presets: schoolEmailService_1.EMAIL_PROVIDER_PRESETS });
});
router.get("/", async (req, res) => {
    try {
        ...
        const settings = await (0, schoolEmailService_1.getPublicSchoolEmailSettings)(schoolId);
        return res.json({
            success: true,
            settings,
        });
...
router.put("/", async (req, res) => {
    try {
        ...
        const result = await (0, schoolEmailService_1.saveSchoolEmailSettings)(schoolId, input);
        if (!result.ok) {
...
router.post("/test", async (req, res) => {
    try {
        ...
        const result = await (0, schoolEmailService_1.testSchoolEmailConnection)(schoolId, testTo || undefined);
        if (!result.ok) {
        ...
        const settings = result.settings ?? (await (0, schoolEmailService_1.getPublicSchoolEmailSettings)(schoolId));
        return res.json({
            success: true,
