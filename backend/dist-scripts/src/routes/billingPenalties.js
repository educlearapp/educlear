"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = require("express");
const prisma_1 = require("../prisma");
const billingSettings_1 = require("./billingSettings");
const billingSettingsEngine_1 = require("../utils/billingSettingsEngine");
const learnerIdentity_1 = require("../utils/learnerIdentity");
const billingLedgerStore_1 = require("../utils/billingLedgerStore");
const router = (0, express_1.Router)();
router.post("/preview", async (req, res) => {
    try {
        const body = (req.body ?? {});
        const schoolId = String(body.schoolId || "").trim();
        const penaltyDate = String(body.penaltyDate || new Date().toISOString()).slice(0, 10);
        const dueDateCutoff = String(body.dueDateCutoff || penaltyDate).slice(0, 10);
        const excludeNotYetDue = Boolean(body.excludeNotYetDue);
        const applyToAll = body.applyTo !== "selected";
        const selectedAccountNos = Array.isArray(body.selectedAccountNos)
            ? body.selectedAccountNos.map((v) => String(v).trim())
            : [];
        if (!schoolId) {
            return res.status(400).json({ success: false, error: "Missing schoolId" });
        }
        const settings = await (0, billingSettings_1.loadSchoolBillingSettings)(schoolId);
        const penaltyDefaults = (0, billingSettingsEngine_1.resolvePenaltyConfig)(settings);
        const resolvedAmount = (0, billingSettingsEngine_1.resolveConfiguredPenaltyAmount)(settings);
        if (!resolvedAmount) {
            return res.status(400).json({
                success: false,
                error: "No late payment fine amount has been configured. Please set it in Billing Settings.",
            });
        }
        const ledger = (0, billingLedgerStore_1.readSchoolLedger)(schoolId);
        const learners = await prisma_1.prisma.learner.findMany({
            where: { schoolId },
            select: {
                id: true,
                firstName: true,
                lastName: true,
                familyAccount: { select: { accountRef: true } },
            },
        });
        const description = String(body.description || penaltyDefaults.description || "Late payment penalty").trim();
        const rows = learners
            .map((learner) => {
            const accountNo = (0, learnerIdentity_1.resolveLearnerAccountNo)(learner);
            if (!accountNo)
                return null;
            if (!applyToAll && !selectedAccountNos.includes(accountNo))
                return null;
            const { balance, overdueAmount, excludedNotYetDue } = (0, billingLedgerStore_1.computeAccountOverdue)(ledger, learner.id, accountNo, { penaltyDate, dueDateCutoff, excludeNotYetDue });
            if (balance <= 0 || overdueAmount <= 0)
                return null;
            const penaltyId = (0, billingLedgerStore_1.buildPenaltyEntryId)(schoolId, accountNo, penaltyDate, description);
            const alreadyApplied = ledger.some((e) => e.id === penaltyId);
            return {
                learnerId: learner.id,
                accountNo,
                learnerName: `${learner.firstName || ""} ${learner.lastName || ""}`.trim(),
                balance,
                overdueAmount,
                excludedNotYetDue,
                penaltyAmount: resolvedAmount,
                apply: !alreadyApplied,
                duplicate: alreadyApplied,
            };
        })
            .filter(Boolean);
        return res.json({ success: true, rows });
    }
    catch (error) {
        console.error("[billing/late-penalties] POST /preview failed:", error);
        return res.status(500).json({ success: false, error: "Server error" });
    }
});
router.post("/apply", async (req, res) => {
    try {
        const body = (req.body ?? {});
        const schoolId = String(body.schoolId || "").trim();
        const settings = await (0, billingSettings_1.loadSchoolBillingSettings)(schoolId);
        const penaltyDefaults = (0, billingSettingsEngine_1.resolvePenaltyConfig)(settings);
        const penaltyAmount = (0, billingSettingsEngine_1.resolveConfiguredPenaltyAmount)(settings);
        if (!penaltyAmount) {
            return res.status(400).json({
                success: false,
                error: "No late payment fine amount has been configured. Please set it in Billing Settings.",
            });
        }
        const penaltyDate = String(body.penaltyDate || new Date().toISOString()).slice(0, 10);
        const dueDate = String(body.dueDate || body.dueDateCutoff || penaltyDate).slice(0, 10);
        const description = String(body.description || penaltyDefaults.description || "Late payment penalty").trim();
        const reference = String(body.reference || `PEN-${penaltyDate}`).trim();
        const accounts = Array.isArray(body.accounts) ? body.accounts : [];
        if (!schoolId || !accounts.length) {
            return res.status(400).json({ success: false, error: "Missing schoolId or accounts" });
        }
        const ledger = (0, billingLedgerStore_1.readSchoolLedger)(schoolId);
        const applied = [];
        const skipped = [];
        for (const row of accounts) {
            const learnerId = String(row?.learnerId || "").trim();
            const accountNo = String(row?.accountNo || "").trim();
            const apply = row?.apply !== false;
            if (!apply || !accountNo) {
                skipped.push({ accountNo: accountNo || "-", reason: "Not selected" });
                continue;
            }
            const balance = (0, billingLedgerStore_1.calculateBalanceForAccount)(ledger, learnerId, accountNo);
            if (balance <= 0) {
                skipped.push({ accountNo, reason: "Paid up or overpaid" });
                continue;
            }
            const id = (0, billingLedgerStore_1.buildPenaltyEntryId)(schoolId, accountNo, penaltyDate, description);
            if (ledger.some((e) => e.id === id)) {
                skipped.push({ accountNo, reason: "Duplicate penalty for date/description" });
                continue;
            }
            const entry = {
                id,
                schoolId,
                learnerId,
                accountNo,
                type: "penalty",
                amount: penaltyAmount,
                date: penaltyDate,
                dueDate,
                reference,
                description,
                createdAt: new Date().toISOString(),
            };
            (0, billingLedgerStore_1.appendSchoolEntry)(schoolId, entry);
            ledger.push(entry);
            applied.push(entry);
        }
        return res.json({ success: true, applied, skipped });
    }
    catch (error) {
        console.error("[billing/late-penalties] POST /apply failed:", error);
        return res.status(500).json({ success: false, error: "Server error" });
    }
});
exports.default = router;
