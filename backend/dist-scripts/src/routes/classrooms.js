...
exports.isUnregisteredClassroomId = isUnregisteredClassroomId;
exports.classNameFromUnregisteredId = classNameFromUnregisteredId;
exports.rebuildMissingClassroomsFromLearners = rebuildMissingClassroomsFromLearners;
...
function normalizeTeacherEmail(raw) {
    return (0, staffJwt_1.normalizeStaffEmail)(String(raw ?? ""));
}
function normalizeTeacherName(raw) {
    return String(raw ?? "").trim();
}
function unregisteredClassroomId(className) {
    return `${UNREGISTERED_PREFIX}${encodeURIComponent(className)}`;
}
function isUnregisteredClassroomId(id) {
    return String(id || "").startsWith(UNREGISTERED_PREFIX);
}
function classNameFromUnregisteredId(id) {
    return decodeURIComponent(String(id).slice(UNREGISTERED_PREFIX.length));
}
function formatClassroomRow(classroom, extras) {
    return {
        ...classroom,
...
async function distinctLearnerClassNames(schoolId) {
    const grouped = await prisma_1.prisma.learner.groupBy({
        by: ["className"],
        where: { ...(0, learnerEnrollment_1.activeLearnerWhere)(schoolId), className: { not: "" } },
        _count: { _all: true },
    });
    return grouped
...
async function learnerCountForClass(schoolId, className) {
    return prisma_1.prisma.learner.count({
        where: { ...(0, learnerEnrollment_1.activeLearnerWhere)(schoolId), className },
    });
}
/** Create Classroom rows for every distinct learner className missing from the Classroom table. */
async function rebuildMissingClassroomsFromLearners(schoolId) {
    const names = await distinctLearnerClassNames(schoolId);
    const existing = await prisma_1.prisma.classroom.findMany({
        where: { schoolId },
        select: { name: true },
    });
    ...
    for (const name of names) {
        ...
        created.push(name);
        await (0, parentPortalService_1.syncParentThreadsForClassroom)(schoolId, classroom.id);
    }
    return { created: created.length, names: created };
}
const router = (0, express_1.Router)();
async function classroomWithLearners(schoolId, classroomId) {
...
router.get("/", async (req, res) => {
    try {
        ...
        const rows = await prisma_1.prisma.classroom.findMany({
            where: { schoolId },
            orderBy: { name: "asc" },
        });
        const registeredByName = new Map();
        for (const c of rows)
            registeredByName.set(c.name, c);
        const classrooms = await Promise.all(rows.map(async (c) => {
            const learners = await prisma_1.prisma.learner.findMany({
        ...
        for (const className of learnerClassNames) {
            if (registeredByName.has(className))
                continue;
...
router.post("/repair-missing", async (req, res) => {
    try {
        ...
        const threads = await (0, parentPortalService_1.repairAllParentTeacherThreads)({ schoolId });
        return res.json({
            success: true,
            classrooms: rebuild,
            threads,
...
router.put("/:id", async (req, res) => {
    try {
        ...
        const existing = await prisma_1.prisma.classroom.findFirst({ where: { id, schoolId } });
        if (!existing)
            return res.status(404).json({ error: "Classroom not found" });
        const name = req.body?.name != null ? String(req.body.name).trim() : existing.name;
        const teacherName = req.body?.teacher != null || req.body?.teacherName != null
            ? normalizeTeacherName(req.body?.teacher ?? req.body?.teacherName)
            : existing.teacherName;
        const teacherEmail = req.body?.teacherEmail != null
            ? normalizeTeacherEmail(req.body.teacherEmail)
            : existing.teacherEmail;
        const classroom = await prisma_1.prisma.classroom.update({
            where: { id },
            data: {
                name,
                teacherName,
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        await prisma_1.prisma.classroom.deleteMany({ where: { id, schoolId } });
...
router.post("/:id/add-learners", async (req, res) => {
    try {
        const schoolId = String(req.body?.schoolId || "").trim();
        const learnerIds = Array.isArray(req.body?.learnerIds) ? req.body.learnerIds : [];
        const id = String(req.params.id);
        let classroomName = "";
        if (isUnregisteredClassroomId(id)) {
...
router.post("/:id/move-learners", async (req, res) => {
    try {
        ...
        const learnerIds = Array.isArray(req.body?.learnerIds) ? req.body.learnerIds : [];
        const target = await prisma_1.prisma.classroom.findFirst({ where: { id: targetId, schoolId } });
        if (!target)
            return res.status(404).json({ error: "Target classroom not found" });
        await prisma_1.prisma.learner.updateMany({
            where: { schoolId, id: { in: learnerIds } },
