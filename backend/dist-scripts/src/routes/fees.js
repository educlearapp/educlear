...
function decodeFeesMeta(rawGrade) {
    const s = asTrimmedString(rawGrade);
    if (!s.startsWith(FEES_META_PREFIX))
        return null;
...
function mapUiTypeToLegacyFrequency(uiType) {
    const v = asTrimmedString(uiType).toUpperCase();
    if (!v)
        return "MONTHLY";
    if (v === "ONCE_OFF")
        return "ONCE_OFF";
    if (v.startsWith("MONTHLY"))
        return "MONTHLY";
...
function getRequiredField(body, key) {
...
function feeUsage(usedBillingPlansCount) {
...
function serializeFee(fee, usedBillingPlansCount = 0) {
...
router.get("/", async (req, res) => {
    try {
        ...
        return res.json({
            success: true,
            items: fees.map((f) => serializeFee(f, usedBillingPlansCount)),
            total,
            page,
            pageSize,
        });
    ...
    try {
        ...
        const fee = await prismaAny.feeStructure.create({
            data: {
                ...
                amount,
                name: description,
                frequency: mapUiTypeToLegacyFrequency(uiType),
