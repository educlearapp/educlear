...
async function resolveDaSilvaSchool() {
  ...
  const byName = await prisma.school.findFirst({
    where: { name: { equals: DA_SILVA_SCHOOL_NAME, mode: "insensitive" } },
    select: { id: true, name: true, email: true },
  });
  if (byName) return byName;

  throw new Error(
    `Da Silva Academy school not found (tried id ${DA_SILVA_ACADEMY_SCHOOL_ID}, email ${OWNER_EMAIL}, name ${DA_SILVA_SCHOOL_NAME})`
  );
}
...
async function main(): Promise<void> {
  ...
  if (normalizeAuthEmail(school.email) !== OWNER_EMAIL) {
