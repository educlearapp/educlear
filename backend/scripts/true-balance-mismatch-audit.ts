...
function txnFingerprint(t: ParsedTransaction): TxnFingerprint {
...
function dateRange(txns: ParsedTransaction[]): { min: string | null; max: string | null } {
...
type RootCause =
  | "missingInvoiceHistoryInPrimaryExport"
  | "missingPaymentHistoryInPrimaryExport"
  | "missingMixedHistoryInPrimaryExport"
  | "backupDoesNotExplainVariance"
  | "duplicateTransactionsInPrimary"
  | "noTransactionsInPrimary"
  | "ledgerExceedsAgeAnalysis"
  | "nearMatchUnexplained";

function classifyRootCause(opts: {
  variance: number;
  ageBalance: number;
  primaryLedger: number;
  backupLedger: number | null;
  missingInPrimary: ParsedTransaction[];
  duplicateCount: number;
  primaryTxnCount: number;
}): RootCause {
  const { variance, ageBalance, primaryLedger, backupLedger, missingInPrimary, duplicateCount, primaryTxnCount } =
    opts;

  if (primaryTxnCount === 0) return "noTransactionsInPrimary";
  ...
  if (backupLedger !== null && Math.abs(backupLedger - ageBalance) <= 1) {
    ...
    if (invExplains && missingInv > 0.01) return "missingInvoiceHistoryInPrimaryExport";
    if (payExplains && Math.abs(missingPay) > 0.01) return "missingPaymentHistoryInPrimaryExport";
    if (mixedExplains && Math.abs(missingNet) > 0.01) {
      if (missingInv > 0.01 && Math.abs(missingPay) > 0.01) return "missingMixedHistoryInPrimaryExport";
      if (missingInv > 0.01) return "missingInvoiceHistoryInPrimaryExport";
      if (Math.abs(missingPay) > 0.01) return "missingPaymentHistoryInPrimaryExport";
    }
  ...
  if (duplicateCount > 0) return "duplicateTransactionsInPrimary";
...
const allMismatches: MismatchRow[] = JSON.parse(fs.readFileSync(mismatchPath, "utf8"));
const targets = allMismatches.filter((r) => r.auditReason === "trueBalanceMismatch");

const txnDir = path.join(desktopRoot, "01_transaction_list");
const primaryPath = path.join(txnDir, "transaction_list.xls");
const backupCandidates = [
  path.join(txnDir, "transaction_list_2023_backup.xls"),
  path.join(txnDir, "transaction_list_2025_backup.xls"),
];
const backupPath = backupCandidates.find((p) => fs.existsSync(p)) || null;

const primaryTxns = bundle.transactions;
const backupTxns = backupPath ? parseTransactionListFile(backupPath) : [];
...
const activeByAccount = countActiveLearnersPerAccount(classLearners, bundle.accounts, familyIndex);
...
type AccountAudit = {
  ...
  variance: number;
  primaryTxnCount: number;
  backupTxnCount: number;
  primaryLedger: number;
  backupLedger: number | null;
  invoiceTotalPrimary: number;
  paymentTotalPrimary: number;
  missingInPrimaryCount: number;
  missingInvoiceSum: number;
  missingPaymentSum: number;
  missingNetSum: number;
  duplicateFingerprints: number;
  dateRangePrimary: { min: string | null; max: string | null };
  dateRangeBackup: { min: string | null; max: string | null };
  rootCause: RootCause;
  backupClosesGap: boolean;
};
...
for (const row of targets) {
  ...
  const backupClosesGap =
    backupLedger !== null && Math.abs(backupLedger - row.ageAnalysisBalance) <= 1;
...
for (const a of audits) causeCounts[a.rootCause]++;
...
const report = {
  ...
  transactionFiles: {
    ...
    note:
      "User asked for transaction_list_2023_backup.xls; on disk: " +
      (backupPath ? path.basename(backupPath) : "no backup found"),
  },
  exportStats: {
...
const outJson = path.join(__dirname, "..", "true-balance-mismatch-audit.json");
const outTxt = path.join(__dirname, "..", "true-balance-mismatch-audit.txt");
...
const lines: string[] = [
  ...
  `  Backup file used: ${backupPath ? path.basename(backupPath) : "NONE"}`,
  ...
  ...sortedCauses.map(
    ([cause, n]) =>
      `  ${cause}: ${n} (${Math.round((n / targets.length) * 1000) / 10}%)`
  ),
  "",
  ...
  ...audits.slice(0, 15).map(
    (a) =>
      `${a.accountNo} | ${a.ageAnalysisBalance} | ${a.ledgerBalanceFromImport} | ${a.variance} | ${a.primaryTxnCount} | ${a.missingInPrimaryCount} | ${a.rootCause}`
  ),
];
