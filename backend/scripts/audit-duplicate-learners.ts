...
async function main() {
  ...
  const byNameClass = new Map<string, typeof learners>();
  ...
  const byNameOnly = new Map<string, typeof learners>();
  for (const l of learners) {
  ...
  const adrien = learners.filter(
    (l) => norm(l.firstName).includes("adrien") && norm(l.lastName).includes("silva")
  );
