...
type SchoolStub = { id: string; name: string; email: string | null; createdAt: Date };
...
type Plan = {
  ...
  resolved: {
    ...
    ownerEmail: string;
    matchedSchools: Array<{ id: string; name: string; email: string | null; createdAt: string }>;
    scopeSchoolIds: string[];
    scopeSchoolRows: Array<{ id: string; name: string; email: string | null; createdAt: string }>;
    canonicalJsonSchoolIds: string[];
  };
  json: {
    ledgerEntryIds: string[];
    kidesysHistoryRowIds: string[];
    billingPlanLearnerIds: string[];
    userAccessUserIds: string[];
  };
  ids: {
    users: Array<{ id: string; email: string }>;
    roles: string[];
    rolePermissions: string[];
    userPermissionOverrides: string[];
    learners: string[];
    parents: string[];
    parentLearnerLinks: string[];
    familyAccounts: string[];
    classrooms: string[];
    billingDeposits: string[];
    bankStatementImports: string[];
    bankTransactions: string[];
    supplierInvoices: string[];
    supplierInvoicePayments: string[];
    supplierInvoiceLines: string[];
    suppliers: string[];
    expenseCategories: string[];
    accountingJournals: string[];
    accountingJournalLines: string[];
    payrollRuns: string[];
    payrollRunEmployees: string[];
    payrollItems: string[];
    payslips: string[];
    payrollSettings: string[];
    payrollEmailLogs: string[];
    letters: string[];
    letterTemplates: string[];
    learnerIncidents: string[];
    learnerResults: string[];
    learnerReports: string[];
    homeworkPosts: string[];
    schoolNotices: string[];
    parentDocuments: string[];
    parentOnboardings: string[];
    parentOutreachQueue: string[];
    parentNotifications: string[];
    parentTeacherThreads: string[];
    parentTeacherMessages: string[];
    communicationCampaigns: string[];
    communicationMessages: string[];
    communicationRecipients: string[];
    communicationLogs: string[];
    communicationTemplates: string[];
    pushSubscriptions: string[];
    schoolEmailSettings: string[];
    schoolCommunicationProfiles: string[];
    teacherPerformances: string[];
    schoolSubscriptions: string[];
    subscriptionInvoices: string[];
    subscriptionPaymentLogs: string[];
    creditPurchaseInvoices: string[];
    creditPurchasePaymentLogs: string[];
    billingSettings: string[];
    feeStructures: string[];
    schoolFeeSettings: string[];
  };
  counts: {
    schoolsToDelete: number;
    prisma: CountMap;
    jsonStores: CountMap;
    migrationArtifacts: CountMap;
  };
};
...
function parseCli(): Cli {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--schoolId") {
      ...
      if (!schoolId) throw new Error("--schoolId requires a value");
      i += 1;
      continue;
    }
...
function looksLikeDaSilvaAcademy(name: string): boolean {
  return name.toLowerCase().includes(SCHOOL_NAME_NEEDLE.toLowerCase());
}

async function resolveCandidateSchools(cliSchoolId?: string): Promise<SchoolStub[]> {
  if (cliSchoolId) {
    ...
    if (!looksLikeDaSilvaAcademy(row.name)) {
      throw new Error(
        `School name must contain "${SCHOOL_NAME_NEEDLE}" for this reset, got "${row.name}". Refusing.`
      );
    }
    return [row];
  }
  ...
  const merged = new Map<string, SchoolStub>();
  for (const s of [...byName, ...byEmail]) merged.set(s.id, s);
  ...
  if (ownerUser?.school) merged.set(ownerUser.school.id, ownerUser.school);

  const out = Array.from(merged.values()).filter((s) => !isPlatformSchoolName(s.name));
  return out;
}

async function resolveScopeSchools(candidates: SchoolStub[]): Promise<{
  platform: { id: string; name: string } | null;
  matched: SchoolStub[];
  scopeSchoolIds: string[];
  scopeSchoolRows: SchoolStub[];
  canonicalJsonSchoolIds: string[];
}> {
  ...
  if (!candidates.length) {
    throw new Error(
      `No Da Silva Academy candidate schools found by name/email. Supply --schoolId if the row exists under another name.`
    );
  }
  ...
  for (const c of candidates) {
    ...
    if (!looksLikeDaSilvaAcademy(c.name) && String(c.email || "").toLowerCase() !== OWNER_EMAIL.toLowerCase()) {
      throw new Error(
        `Refusing: candidate school "${c.name}" (${c.id}) does not look like Da Silva Academy.`
      );
    }
  }

  const primary = candidates[0];
  const scopeSchoolIds = await collectDaSilvaSchoolIds(prisma, [primary.id]);
...
async function collectIdsForSchool(schoolId: string): Promise<Plan["ids"]> {
  const pickIds = async (fn: () => Promise<Array<{ id: string }>>): Promise<string[]> =>
    (await fn()).map((r) => r.id);
  ...
  const ids: Plan["ids"] = {
    ...
    parentLearnerLinks: await pickIds(() =>
      prisma.parentLearnerLink.findMany({ where: { schoolId }, select: { id: true } })
    ),
    familyAccounts: await pickIds(() =>
      prisma.familyAccount.findMany({ where: { schoolId }, select: { id: true } })
    ),
    classrooms: await pickIds(() => prisma.classroom.findMany({ where: { schoolId }, select: { id: true } })),
    billingDeposits: await pickIds(() =>
      prisma.billingDeposit.findMany({ where: { schoolId }, select: { id: true } })
    ),
    bankStatementImports: await pickIds(() =>
      prisma.bankStatementImport.findMany({ where: { schoolId }, select: { id: true } })
    ),
    bankTransactions: await pickIds(() =>
      prisma.bankTransaction.findMany({ where: { schoolId }, select: { id: true } })
    ),
    supplierInvoices: await pickIds(() =>
      prisma.supplierInvoice.findMany({ where: { schoolId }, select: { id: true } })
    ),
    supplierInvoicePayments: await pickIds(() =>
      prisma.supplierInvoicePayment.findMany({
        where: { invoice: { schoolId } },
        select: { id: true },
      })
    ),
    supplierInvoiceLines: await pickIds(() =>
    ...
    expenseCategories: await pickIds(() =>
      prisma.expenseCategory.findMany({ where: { schoolId }, select: { id: true } })
    ),
    accountingJournals: await pickIds(() =>
      prisma.accountingJournal.findMany({ where: { schoolId }, select: { id: true } })
    ),
    accountingJournalLines: await pickIds(() =>
    ...
    payrollRunEmployees: await pickIds(() =>
      prisma.payrollRunEmployee.findMany({
        where: { payrollRun: { schoolId } },
        select: { id: true },
      })
    ),
    payrollItems: await pickIds(() =>
    ...
    payrollSettings: await pickIds(() =>
      prisma.payrollSetting.findMany({ where: { schoolId }, select: { id: true } })
    ),
    payrollEmailLogs: await pickIds(() =>
      prisma.payrollEmailLog.findMany({ where: { schoolId }, select: { id: true } })
    ),
    letters: await pickIds(() => prisma.letter.findMany({ where: { schoolId }, select: { id: true } })),
    letterTemplates: await pickIds(() =>
      prisma.letterTemplate.findMany({ where: { schoolId }, select: { id: true } })
    ),
    learnerIncidents: await pickIds(() =>
      prisma.learnerIncident.findMany({ where: { schoolId }, select: { id: true } })
    ),
    learnerResults: await pickIds(() =>
      prisma.learnerResult.findMany({ where: { schoolId }, select: { id: true } })
    ),
    learnerReports: await pickIds(() =>
      prisma.learnerReport.findMany({ where: { schoolId }, select: { id: true } })
    ),
    homeworkPosts: await pickIds(() =>
      prisma.homeworkPost.findMany({ where: { schoolId }, select: { id: true } })
    ),
    schoolNotices: await pickIds(() =>
      prisma.schoolNotice.findMany({ where: { schoolId }, select: { id: true } })
    ),
    parentDocuments: await pickIds(() =>
      prisma.parentDocument.findMany({ where: { schoolId }, select: { id: true } })
    ),
    parentOnboardings: await pickIds(() =>
      prisma.parentOnboarding.findMany({ where: { schoolId }, select: { id: true } })
    ),
    parentOutreachQueue: await pickIds(() =>
      prisma.parentOutreachQueue.findMany({ where: { schoolId }, select: { id: true } })
    ),
    parentNotifications: await pickIds(() =>
      prisma.parentNotification.findMany({ where: { schoolId }, select: { id: true } })
    ),
    parentTeacherThreads: await pickIds(() =>
      prisma.parentTeacherThread.findMany({ where: { schoolId }, select: { id: true } })
    ),
    parentTeacherMessages: await pickIds(() =>
      prisma.parentTeacherMessage.findMany({ where: { schoolId }, select: { id: true } })
    ),
    communicationCampaigns: await pickIds(() =>
      prisma.communicationCampaign.findMany({ where: { schoolId }, select: { id: true } })
    ),
    communicationMessages: await pickIds(() =>
      prisma.communicationMessage.findMany({ where: { schoolId }, select: { id: true } })
    ),
    communicationRecipients: await pickIds(() =>
      prisma.communicationRecipient.findMany({
        where: { message: { schoolId } },
        select: { id: true },
      })
    ),
    communicationLogs: await pickIds(() =>
      prisma.communicationLog.findMany({ where: { schoolId }, select: { id: true } })
    ),
    communicationTemplates: await pickIds(() =>
      prisma.communicationTemplate.findMany({ where: { schoolId }, select: { id: true } })
    ),
    pushSubscriptions: await pickIds(() =>
      prisma.pushSubscription.findMany({ where: { schoolId }, select: { id: true } })
    ),
    schoolEmailSettings: await pickIds(() =>
      prisma.schoolEmailSettings.findMany({ where: { schoolId }, select: { id: true } })
    ),
    schoolCommunicationProfiles: await pickIds(() =>
      prisma.schoolCommunicationProfile.findMany({ where: { schoolId }, select: { id: true } })
    ),
    teacherPerformances: await pickIds(() =>
      prisma.teacherPerformance.findMany({ where: { schoolId }, select: { id: true } })
    ),
    schoolSubscriptions: await pickIds(() =>
      prisma.schoolSubscription.findMany({ where: { schoolId }, select: { id: true } })
    ),
    subscriptionInvoices: await pickIds(() =>
      prisma.subscriptionInvoice.findMany({ where: { schoolId }, select: { id: true } })
    ),
    subscriptionPaymentLogs: await pickIds(() =>
      prisma.subscriptionPaymentLog.findMany({ where: { schoolId }, select: { id: true } })
    ),
    creditPurchaseInvoices: await pickIds(() =>
      prisma.creditPurchaseInvoice.findMany({ where: { schoolId }, select: { id: true } })
    ),
    creditPurchasePaymentLogs: await pickIds(() =>
      prisma.creditPurchasePaymentLog.findMany({ where: { schoolId }, select: { id: true } })
    ),
    billingSettings: await pickIds(() =>
      prisma.billingSettings.findMany({ where: { schoolId }, select: { id: true } })
    ),
    feeStructures: await pickIds(() =>
      prisma.feeStructure.findMany({ where: { schoolId }, select: { id: true } })
    ),
    schoolFeeSettings: await pickIds(() =>
      prisma.schoolFeeSetting.findMany({ where: { schoolId }, select: { id: true } })
    ),
  };
...
function removeStagingDir(dir: string): number {
  ...
  if (files && fs.existsSync(dir)) fs.rmSync(dir, { recursive: true, force: true });
  return files;
}
...
function purgeMigrationFileArtifacts(scopeSchoolIds: string[]): CountMap {
  const removed: CountMap = {};
  ...
  for (const runbook of listRunbooks().filter((r) => idSet.has(String(r.schoolId || "")))) {
    const filePath = path.join(process.cwd(), "storage", "migration-runbooks", `${runbook.runbookId}.json`);
    if (fs.existsSync(filePath)) {
      fs.unlinkSync(filePath);
  ...
  for (const pack of listSignoffs().filter((s) => idSet.has(String(s.schoolId || "")))) {
    const filePath = path.join(process.cwd(), "storage", "migration-signoffs", `${pack.signoffId}.json`);
    if (fs.existsSync(filePath)) {
      fs.unlinkSync(filePath);
  ...
  if (fs.existsSync(MIGRATION_STAGING_ROOT)) {
    for (const entry of fs.readdirSync(MIGRATION_STAGING_ROOT, { withFileTypes: true })) {
      ...
      if (entry.isFile() && scopeSchoolIds.some((sid) => entry.name.includes(sid)) && fs.existsSync(full)) {
        fs.unlinkSync(full);
...
async function buildPlan(cli: Cli): Promise<{ plan: Plan; backupPath: string }> {
  const candidates = await resolveCandidateSchools(cli.schoolId);
  const scope = await resolveScopeSchools(candidates);

  const purgeScope = await buildDaSilvaPurgeScope(prisma, scope.scopeSchoolRows[0]?.id || candidates[0].id);

  const prismaCounts: CountMap = {};
  const allIds: Plan["ids"] = {
    users: [],
    roles: [],
    rolePermissions: [],
  ...
  for (const s of scope.scopeSchoolRows) {
    ...
    for (const key of Object.keys(allIds) as Array<keyof Plan["ids"]>) {
      if (key === "users") continue;
      // @ts-expect-error index
      allIds[key].push(...(ids[key] as string[]));
    }
  }

  const countKey = (key: string, n: number) => {
  ...
  const migrationCounts: CountMap = {};
  bump(migrationCounts, "migrationImportBatches", listImportBatches().filter((b) => scope.scopeSchoolIds.includes(String(b.targetSchoolId || ""))).length);
  bump(migrationCounts, "migrationStages", listStages().filter((s) => scope.scopeSchoolIds.some((sid) => stageBelongsToSchool(s.stageId, sid))).length);
  bump(migrationCounts, "migrationPilots", listPilots().filter((p) => scope.scopeSchoolIds.includes(String(p.schoolId || ""))).length);
  bump(migrationCounts, "migrationRunbooks", listRunbooks().filter((r) => scope.scopeSchoolIds.includes(String(r.schoolId || ""))).length);
  bump(migrationCounts, "migrationSignoffs", listSignoffs().filter((s) => scope.scopeSchoolIds.includes(String(s.schoolId || ""))).length);
  for (const sid of scope.scopeSchoolIds) {
    bump(migrationCounts, "migrationStagingUploadFiles", countStagingFiles(path.join(STAGING_UPLOAD_ROOT, sid)));
  }

  const jsonCounts: CountMap = {};
  const ledgerEntryIds: string[] = [];
  const kidesysHistoryRowIds: string[] = [];
  const billingPlanLearnerIds: string[] = [];
  ...
  for (const sid of purgeScope.schoolIds) {
    ...
    for (const row of readSchoolKidesysHistory(sid)) {
      ...
      if (id) kidesysHistoryRowIds.push(id);
    }
  ...
  const userAccessUserIds: string[] = [];
  const userAccessPath = path.join(DATA_DIR, "user-access.json");
  if (fs.existsSync(userAccessPath)) {
    const parsed = JSON.parse(fs.readFileSync(userAccessPath, "utf8")) as {
    ...
    for (const [uid, meta] of Object.entries(users)) {
      if (schoolIdSet.has(String(meta.schoolId || "").trim())) userAccessUserIds.push(uid);
    }
  }
  ...
  const dir = path.join(process.cwd(), "uploads", "reset-backups");
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  const ts = generatedAt.replace(/[:.]/g, "-");
  const backupPath = path.join(dir, `da-silva-reset-${ts}.json`);
...
async function verifyAfter(scopeSchoolRowIds: string[]): Promise<{ passed: boolean; issues: string[] }> {
  const issues: string[] = [];
  const platform = await prisma.school.findFirst({
    where: { name: { equals: PLATFORM_SCHOOL_NAME, mode: "insensitive" } },
    select: { id: true, name: true },
  });
  if (!platform) issues.push(`EduClear Platform school missing (${PLATFORM_SCHOOL_NAME})`);
  ...
  if (ownerUser) issues.push(`Owner user still exists: ${OWNER_EMAIL} (${ownerUser.id})`);

  // Ensure no rows remain for deleted school ids.
  const checkCount = async (label: string, n: number) => {
    if (n !== 0) issues.push(`${label} remaining for deleted school(s): ${n}`);
  };
  await checkCount(
    "learners",
    await prisma.learner.count({ where: { schoolId: { in: scopeSchoolRowIds } } })
  );
  await checkCount(
    "parents",
    await prisma.parent.count({ where: { schoolId: { in: scopeSchoolRowIds } } })
  );
  await checkCount(
    "family accounts",
    await prisma.familyAccount.count({ where: { schoolId: { in: scopeSchoolRowIds } } })
  );
  await checkCount(
    "classrooms",
    await prisma.classroom.count({ where: { schoolId: { in: scopeSchoolRowIds } } })
  );
...
async function main(): Promise<void> {
  ...
  if (outOfScope.length) {
    throw new Error(
      `Refusing: ${OWNER_EMAIL} exists under a different school outside scope: ${outOfScope
  ...
  const removed: Record<string, CountMap> = { prisma: {}, subscription: {}, migration: {}, json: {} };

  const purgeScope = await buildDaSilvaPurgeScope(prisma, scopeSchoolRowIds[0]);
  ...
  for (const school of scope.scopeSchoolRows) {
    ...
    for (const [k, v] of Object.entries(subscriptionRemoved)) bump(removed.subscription, k, v);
    ...
    for (const [k, v] of Object.entries(purged)) bump(removed.prisma, k, v);
    ...
    try {
      ...
      throw new Error(
        `Failed to delete school row ${school.id} (${school.name}). ${e instanceof Error ? e.message : String(e)}`
      );
    }
  }

  const migrationRemoved = purgeMigrationFileArtifacts(scope.scopeSchoolIds);
  for (const [k, v] of Object.entries(migrationRemoved)) bump(removed.migration, k, v);

  const jsonRemoved = purgeDaSilvaJsonStores(purgeScope);
  for (const [k, v] of Object.entries(jsonRemoved)) bump(removed.json, k, v);
  ...
  console.log(
    `Deleted billing/history (JSON): ledger=${removed.json.billingLedgerEntries || 0}, plans=${removed.json.learnerBillingPlanLearners || 0}, history=${removed.json.kidesysHistoryRows || 0}`
  );
