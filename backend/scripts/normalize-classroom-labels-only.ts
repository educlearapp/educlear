...
const EXPLICIT_CLASS_REMAP: Record<string, string> = {
  "|grade Ra": "Grade RA",
  "|grade Rb": "Grade RB",
  "lgrade Ra": "Grade RA",
  "lgrade Rb": "Grade RB",
...
export function canonicalFromClassListRaw(rawClassName: string): string {
  ...
  if (/^grade\s+r[a-z]{1,2}$/i.test(raw)) {
    ...
    if (m) return `Grade R${m[1].toUpperCase()}`;
  }
...
function parseExplicitClassLists(classListDir: string): {
  fileCanonicalByLearner: Map<string, string>;
  fileCanonicalByMatchKey: Map<string, string>;
  classrooms: Array<{ className: string; canonical: string; sourceFile: string; rawCount: number }>;
} {
  const fileCanonicalByLearner = new Map<string, string>();
  const fileCanonicalByMatchKey = new Map<string, string>();
...
async function main(): Promise<void> {
  ...
  const classListDir =
    args[2] || path.join(process.env.HOME || "", "Desktop", "05_class_list");
  ...
  const activeFromFiles: ParsedLearner[] = [];
  for (const file of CLASS_LIST_FILES) {
    activeFromFiles.push(...parseClassListFile(path.join(classListDir, file)).learners);
  }
  const activeUnique = uniqueLearnersByMatchKey(activeFromFiles);

  const activeByNormName = new Map<string, Array<ParsedLearner & { targetClass: string }>>();
  ...
  const beforeClassroomCount = new Set(
    dbLearners.map((l) => String(l.className || "").trim()).filter(Boolean)
  ).size;

  const beforeByClass: Record<string, number> = {};
  ...
  const billingPlanItemCountBefore = Object.values(readSchoolBillingPlans(schoolId)).reduce(
  ...
  for (const learner of dbLearners) {
    ...
    const matchKey = buildLearnerMatchKey(fullName, current);

    let target =
      fileCanonicalByMatchKey.get(matchKey) ||
      EXPLICIT_CLASS_REMAP[current] ||
      null;
    let reason = "class-list matchKey";

    if (!target) {
      const hits: Array<ParsedLearner & { targetClass: string }> = [];
  ...
  const updatesByTarget: Record<string, number> = {};
  const updatesByFromTo: Record<string, number> = {};
  ...
  const afterByClass: Record<string, number> = {};
  for (const l of afterLearners) {
    const cn = String(l.className || "").trim() || "(null)";
    afterByClass[cn] = (afterByClass[cn] || 0) + 1;
  }

  const afterClassroomCount = new Set(
    afterLearners.map((l) => String(l.className || "").trim()).filter(Boolean)
  ).size;
  ...
  const billingPlanItemCountAfter = Object.values(readSchoolBillingPlans(schoolId)).reduce(
    (sum, items) => sum + (Array.isArray(items) ? items.length : 0),
    0
  );

  const verification = {
    learnerCount396: finalLearnerTotal === DA_SILVA_EXPECTED_LEARNER_COUNT,
    classroomCount21: afterClassroomCount === DA_SILVA_FINAL_IMPORT_EXPECTED.classes,
    noBillingTouch:
    ...
    classListCovers396: activeUnique.length === DA_SILVA_EXPECTED_LEARNER_COUNT,
  ...
  const outJson = path.join(process.cwd(), "classroom-label-normalization-report.json");
  const outTxt = path.join(process.cwd(), "classroom-label-normalization-report.txt");
