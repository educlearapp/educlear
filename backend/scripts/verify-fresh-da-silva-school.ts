...
async function main(): Promise<void> {
  ...
  const learnersBody = (await learnersRes.json().catch(() => [])) as unknown;
  const learnerList = Array.isArray(learnersBody)
    ? learnersBody
    : Array.isArray((learnersBody as { learners?: unknown[] })?.learners)
      ? (learnersBody as { learners: unknown[] }).learners
      : [];
  record(
    "Registrations learners",
    learnersRes.status === 200 && learnerList.length === 0,
    `${learnersRes.status} — count=${learnerList.length}`
  );

  const statementsRes = await fetch(`${API_BASE}/api/statements?${q}`, { headers: auth });
  const statementsBody = (await statementsRes.json().catch(() => ({}))) as {
    statements?: unknown[];
    entries?: unknown[];
  };
  ...
  record(
    "Statements",
    statementsRes.status === 200 && statementRows.length === 0,
    `${statementsRes.status} — rows=${statementRows.length}`
  );

  const paymentsRes = await fetch(`${API_BASE}/api/payments?${q}`, { headers: auth });
  const paymentsBody = (await paymentsRes.json().catch(() => ({}))) as {
    payments?: unknown[];
  };
  ...
  record(
    "Payments",
    paymentsRes.status === 200 && paymentRows.length === 0,
    `${paymentsRes.status} — rows=${paymentRows.length}`
  );

  const migrationRes = await fetch(`${API_BASE}/api/super-admin/migration/target-schools`, {
    headers: auth,
  });
  const migrationBody = (await migrationRes.json().catch(() => ({}))) as {
    schools?: Array<{ id?: string; name?: string }>;
    success?: boolean;
    error?: string;
  };
  const schools = migrationBody.schools || [];
  const hasDaSilva = schools.some((s) =>
    String(s.name || "")
      .toLowerCase()
      .includes("da silva")
  );
  const daSilvaProjectRes = await fetch(
    `${API_BASE}/api/super-admin/migration/da-silva/projects`,
    {
      method: "POST",
  ...
  const daSilvaProjectBody = (await daSilvaProjectRes.json().catch(() => ({}))) as {
    success?: boolean;
    projectId?: string;
  };

  record(
    "Migration Center (target-schools)",
    migrationRes.status === 200 && schools.length > 0 && hasDaSilva,
    `${migrationRes.status} — schools=${schools.length}, daSilvaListed=${hasDaSilva}`
  );
  record(
    "Migration Center (da-silva project)",
    daSilvaProjectRes.status === 200 && daSilvaProjectBody.success === true,
    `${daSilvaProjectRes.status} — projectId=${daSilvaProjectBody.projectId || "(none)"}`
  );
