...
const HISTORICAL_MOVEMENT_NOTE =
  /\b(removed|refund|closed|not returning|relocat|write[\s-]?off|learner left|no longer|cancelled|canceled|credit note|discount|not doing|left school|historical|jamf|cutover|opening)\b/i;

const UNALLOCATED_PAYMENT_NOTE =
  /\b(unalloc|not alloc|advance|on account|suspense|bulk pay|unallocated|prepaid|credit on account|held|float)\b/i;

const RECEIPT_TYPE_NOTE =
  /\b(receipt|eft|deposit|cash|paid|payment|bank|card|snapscan|ozow)\b/i;

type LikelyCause =
  | "normalKidESysAllocationGap"
  | "missingInvoiceItem"
  | "missingPaymentItem"
  | "historicalCutoverOpeningBalanceNeeded"
  | "manualReview";
...
type AccountDeepDive = {
  ...
  hasUnallocatedPaymentSignals: boolean;
  unallocatedSignalDetail: string;
...
function paymentCoveragePercent(invoiceSum: number, paymentSum: number): number | null {
  if (invoiceSum <= 0.01) return null;
  return roundMoney((Math.abs(paymentSum) / invoiceSum) * 100);
}

function detectUnallocatedSignals(txns: ParsedTransaction[]): {
  hasSignals: boolean;
  detail: string;
  receiptStylePaymentCount: number;
  unallocatedNotePaymentCount: number;
  ...
  for (const t of payments) {
    ...
    if (RECEIPT_TYPE_NOTE.test(note) && !/\binvoice\b/i.test(note)) receiptStylePaymentCount++;
    if (UNALLOCATED_PAYMENT_NOTE.test(note)) unallocatedNotePaymentCount++;
  }
...
function classifyLikelyCause(opts: {
...
}): { cause: LikelyCause; confidence: AccountDeepDive["causeConfidence"]; detail: string } {
  const {
    row,
    invoiceCount,
    paymentCount,
  ...
  const ageNotExplainedByExport =
    age > roundMoney(invoiceSum + Math.abs(paymentSum)) + Math.max(300, variance * 0.2);
  ...
  if (
    ...
    const tightMatch =
      row.paymentMatchConfidence === "high" ||
...
const review = JSON.parse(fs.readFileSync(reviewPath, "utf8")) as {
  accountsByReasonGroup: { possibleUnallocatedPayment: ReviewTarget[] };
};

const targets: ReviewTarget[] = (
  review.accountsByReasonGroup?.possibleUnallocatedPayment || []
).map((r) => ({
  ...
  variance: r.variance,
  paymentPatterns: (r as { paymentPatterns?: string[] }).paymentPatterns || [],
  paymentMatchConfidence:
    (r as { paymentMatchConfidence?: "high" | "medium" | "none" }).paymentMatchConfidence || "none",
}));
...
for (const row of targets) {
  ...
  const historicalNoteTxnCount = txns.filter((t) =>
    HISTORICAL_MOVEMENT_NOTE.test(String(t.notes || ""))
  ).length;
  ...
  deepDives.push({
    ...
    metrics: {
      ...
      siblingAccountNos: [...(parentSiblings.get(row.accountNo) || new Set<string>())],
...
for (const d of deepDives) causeCounts[d.likelyCause]++;
...
const outJson = path.join(__dirname, "..", "unallocated-payment-deep-dive.json");
const outTxt = path.join(__dirname, "..", "unallocated-payment-deep-dive.txt");
...
for (const d of deepDives) {
  ...
  lines.push(
    `  total invoices: R${d.totalInvoices} (${d.invoiceCount}) | total payments: R${d.totalPayments} (${d.paymentCount}) | coverage: ${d.paymentCoveragePercent === null ? "n/a" : `${d.paymentCoveragePercent}%`}`
  );
  lines.push(
    `  unallocated signals: ${d.hasUnallocatedPaymentSignals ? "yes" : "no"} — ${d.unallocatedSignalDetail}`
  );
