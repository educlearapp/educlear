...
import { buildAndGenerateStatementPdf } from "../src/services/statementPdfData";
import {
  loadSchoolLogoBuffer,
  resolveUploadsFilePath,
  toStoredSchoolLogoUrl,
} from "../src/utils/schoolLogo";
...
async function main(): Promise<void> {
  ...
  const stored = toStoredSchoolLogoUrl(school.logoUrl);
  ...
  const filePath = logoUrl ? resolveUploadsFilePath(logoUrl) : null;
  console.log("Resolved file path:", filePath ?? "(none)");
  console.log("File exists:", filePath ? fs.existsSync(filePath) : false);

  const buffer = await loadSchoolLogoBuffer(logoUrl);
  ...
  const { buffer: pdfBuffer, input } = await buildAndGenerateStatementPdf({
    schoolId,
