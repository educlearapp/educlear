...
import { describeOwnerProvisioning } from "../src/utils/ownerProvisioning";
...
async function postLogin(url: string, password: string): Promise<{ status: number; body: unknown }> {
...
async function main(): Promise<void> {
  ...
  if (!users.length) {
    ...
    for (const u of users) {
      ...
      const provenance = school
        ? describeOwnerProvisioning(u, school)
        : "unknown";
      add(
        "1-provenance",
        "Owner provisioning source",
        provenance === "registration" ? "pass" : provenance === "script" ? "warn" : "warn",
        school
          ? `${provenance} (school.email=${school.email || "null"}, fullName=${u.fullName || ""})`
          : "school missing"
      );
      if (provenance === "script" && ownerPassword) {
        const match = await bcrypt.compare(ownerPassword, u.passwordHash);
        if (!match) {
          add(
            "1-lost-password",
            "Registration password stored",
            "fail",
            "Owner was script-created — password in env does not match; reclaim via register-school with your chosen password"
          );
        }
        ...
        add(
          ...
          "Registration password stored",
          "warn",
          "Owner was script-created — set REGISTRATION_PASSWORD and run verify-school-registration-auth.ts"
        );
      }
      ...
      add(
        "1-schoolId",
        "schoolId linked to Da Silva Academy",
        schoolOk ? "pass" : "fail",
        `user ${u.id} schoolId=${u.schoolId}`
      );
      add(
        "1-active",
        "User isActive",
        u.isActive ? "pass" : "fail",
        `user ${u.id} isActive=${u.isActive}`
      );
      add(
        "1-role",
        "Prisma role present",
      ...
      add(
        ...
        hashOk ? "pass" : "fail",
        `length=${hash.length} prefix=${hash.slice(0, 7)}`
      );

      if (ownerPassword) {
        const match = await bcrypt.compare(ownerPassword, hash);
        add(
          "3-compare",
          "bcrypt.compare(DA_SILVA_OWNER_PASSWORD)",
          match ? "pass" : "fail",
          match ? "password matches stored hash" : "password does NOT match stored hash"
        );
      } else {
        add(
          "3-compare",
          "bcrypt.compare(DA_SILVA_OWNER_PASSWORD)",
          "skip",
          "Set DA_SILVA_OWNER_PASSWORD in env to test hash match"
        );
      }

      const meta = getUserAccessMeta(u.id);
      if (meta?.appRole === "Owner" && meta.schoolId === DA_SILVA_SCHOOL_ID) {
        add(
          "1-permissions",
          "user-access.json Owner permissions",
          "pass",
          `appRole=${meta.appRole} schoolId=${meta.schoolId}`
        );
      } else if (!meta) {
        add(
          "1-permissions",
          "user-access.json Owner permissions",
          "warn",
          `No user-access meta for ${u.id}; run ensure-da-silva-owner.ts`
        );
      } else {
        add(
          "1-permissions",
          "user-access.json Owner permissions",
          "warn",
          `meta appRole=${meta.appRole} schoolId=${meta.schoolId}`
        );
      }
      ...
      add(
        "1-perm-map",
        "Owner permission map populated",
        metaPermKeys.length >= permKeys.length ? "pass" : "warn",
        `modules in meta: ${metaPermKeys.length}, expected: ${permKeys.length}`
      );
    }
  }
  ...
  add(
    ...
    process.env.JWT_SECRET ? "JWT_SECRET is set" : "JWT_SECRET missing"
  );
  ...
  add(
    ...
    wrongPw.status === 401 ? "pass" : wrongPw.status === 404 ? "fail" : "warn",
    `status=${wrongPw.status} body=${JSON.stringify(wrongPw.body)}`
  );

  const apiAlias = await postLogin(`${API_BASE}/api/auth/login`, "__audit_wrong_password__");
  add(
    "4-api-alias",
    "POST /api/auth/login reachable (alias)",
    apiAlias.status === 401 ? "pass" : apiAlias.status === 404 ? "fail" : "warn",
    `status=${apiAlias.status} body=${JSON.stringify(apiAlias.body)}`
  );
  ...
  if (VERIFY_LOGIN) {
    if (!ownerPassword) {
      add(
        ...
        "LOGIN VERIFIED",
        "fail",
        "--verify-login requires DA_SILVA_OWNER_PASSWORD in env"
      );
    } else {
      ...
      const token =
        ...
        ok.body !== null &&
        "token" in ok.body
          ? String((ok.body as { token?: string }).token || "")
          : "";
      ...
      add(
        ...
        loginVerified
          ? `200 OK, token issued, schoolId=${(ok.body as { user?: { schoolId?: string } }).user?.schoolId}`
    ...
    add(
      ...
      "LOGIN VERIFIED",
      "skip",
      "Re-run with DA_SILVA_OWNER_PASSWORD=... --verify-login after password reset"
    );
  }
  ...
  const report = {
    ...
    checks,
    recovery:
      "REGISTRATION_EMAIL=dasilvaacademy@gmail.com REGISTRATION_PASSWORD='...' npx tsx scripts/verify-school-registration-auth.ts --apply-reclaim --verify-login (preferred — restores registration password, not a reset)",
  };

  const outJson = path.join(process.cwd(), "login-auth-audit.json");
  const outTxt = path.join(process.cwd(), "login-auth-audit.txt");
