...
type PatternFlag =
  | "unallocatedPaymentPattern"
  | "crossAccountCreditOnLinked"
  | "paymentsWithoutInvoiceCoverage"
  | "negativeLedgerPositiveAge"
  | "familyLinkedCreditOffset"
  | "historicalReceiptOrPaymentOnly";
...
type AccountAudit = {
  ...
  primaryExplanation: PatternFlag | "multiPattern" | "unexplained";
  explainedByPaymentTheory: boolean;
  explanationConfidence: "high" | "medium" | "low" | "none";
  explanationNote: string;
  metrics: {
    rawLedgerOnAccountNo: number;
    familyMappedLedger: number;
    invoiceSum: number;
    paymentSum: number;
    paymentCount: number;
    invoiceCount: number;
    crossAccountPaymentAmount: number;
    crossAccountInboundPaymentAmount: number;
    misPostedTxnCount: number;
    siblingAccountNos: string[];
    siblingCombinedAge: number;
    siblingCombinedLedger: number;
    siblingCombinedVariance: number;
    familyVarianceImprovement: number;
    paymentToInvoiceRatio: number;
    receiptOnlyPaymentCount: number;
    invoiceOnlyPeriod: boolean;
  };
};
...
function sumPayments(txns: ParsedTransaction[]): number {
  return Math.round(
    txns.filter((t) => t.kind === "payment").reduce((s, t) => s + t.signedAmount, 0) * 100
  ) / 100;
}

function sumInvoices(txns: ParsedTransaction[]): number {
  return Math.round(
    txns.filter((t) => t.kind === "invoice").reduce((s, t) => s + t.signedAmount, 0) * 100
  ) / 100;
}
...
function near(a: number, b: number, tol = 50): boolean {
...
function classifyAccount(
  ...
  index: FamilyAccountIndex,
  siblingNos: string[],
  reconciliationByAccount: Map<string, { age: number; ledger: number; variance: number }>
): AccountAudit {
  ...
  for (const t of txnsOnAccount) {
    ...
    if (family && family !== accountNo) {
      misPostedTxnCount++;
      if (t.kind === "payment") crossAccountPaymentAmount += Math.abs(t.signedAmount);
    }
  }
  ...
  const paymentToInvoiceRatio =
    invoiceSum > 0.01 ? Math.round((Math.abs(paymentSum) / invoiceSum) * 1000) / 1000 : 0;

  const receiptOnlyPaymentCount = txnsOnAccount.filter(
    (t) =>
      t.kind === "payment" &&
      /\b(receipt|eft|deposit|cash|paid|payment)\b/i.test(String(t.notes || "")) &&
      !/\binvoice\b/i.test(String(t.notes || ""))
  ).length;
  ...
  const familyLinkedCreditOffset =
    siblingList.length > 0 &&
  ...
  if (familyLinkedCreditOffset) patterns.push("familyLinkedCreditOffset");

  const historicalReceiptOrPaymentOnly =
    (paymentCount > 0 && invoiceCount === 0) ||
  ...
  if (historicalReceiptOrPaymentOnly) patterns.push("historicalReceiptOrPaymentOnly");

  let explainedByPaymentTheory = false;
  let explanationConfidence: AccountAudit["explanationConfidence"] = "none";
  let explanationNote = "";
  let primaryExplanation: AccountAudit["primaryExplanation"] = "unexplained";
...
const contacts = parseContactListFile(
  path.join(desktopRoot, "04_contact_list", "contact_list.xls")
);

const mismatchPath = path.join(__dirname, "..", "active-age-mismatches.json");
const allRows: TargetRow[] = JSON.parse(fs.readFileSync(mismatchPath, "utf8"));
const targets = allRows.filter(
  (r) =>
    r.auditReason === "trueBalanceMismatch" ||
...
const audits: AccountAudit[] = targets.map((row) => {
  ...
  const siblings = parentSiblings.get(row.accountNo) || new Set<string>();
...
function countPattern(flag: PatternFlag): number {
...
const byReason = {
  ...
  possibleDuplicateOrHistoricalCredit: audits.filter(
    (a) => a.auditReason === "possibleDuplicateOrHistoricalCredit"
  ),
};
...
const report = {
  ...
  explanationEstimates: {
    ...
    pctExplainedHighOrMedium:
      Math.round(((explainedHigh.length + explainedMedium.length) / targets.length) * 1000) / 10,
    trueBalanceMismatchExplained: byReason.trueBalanceMismatch.filter((a) => a.explainedByPaymentTheory)
      .length,
    creditRowExplained: byReason.possibleDuplicateOrHistoricalCredit.filter(
      (a) => a.explainedByPaymentTheory
    ).length,
  },
  crossAccountStats: {
    ...
    accountsWithInboundCrossPayments: audits.filter(
      (a) => a.metrics.crossAccountInboundPaymentAmount > 0
    ).length,
    totalCrossAccountOutbound: Math.round(
      audits.reduce((s, a) => s + a.metrics.crossAccountPaymentAmount, 0) * 100
    ) / 100,
    totalCrossAccountInbound: Math.round(
      audits.reduce((s, a) => s + a.metrics.crossAccountInboundPaymentAmount, 0) * 100
    ) / 100,
  },
  audits: audits.sort((a, b) => Math.abs(b.variance) - Math.abs(a.variance)),
};

const outJson = path.join(__dirname, "..", "payment-allocation-mismatch-audit.json");
const outTxt = path.join(__dirname, "..", "payment-allocation-mismatch-audit.txt");
...
for (const a of report.audits.slice(0, 25)) {
  lines.push(
    `${a.accountNo.padEnd(9)} | ${a.auditReason.slice(0, 12).padEnd(12)} | ${String(a.variance).padStart(8)} | ${a.patterns.join(",") || "-"} | ${a.explainedByPaymentTheory ? a.explanationConfidence : "no"}`
  );
}
