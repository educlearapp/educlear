...
import {
  applyMigrationBillingPlansImport,
  previewMigrationBillingPlansImport,
} from "../services/migrationCentre/billingPlansImportService";
...
router.post("/preview", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    if (!file) {
      return jsonError(res, 400, "Upload billing plan summary (.xls, .xlsx, or .csv)");
    }
    ...
    tmpPath = file.path;
    const preview = await previewMigrationBillingPlansImport({
      schoolId,
      billingPlanFilePath: tmpPath,
      originalFileName: file.originalname,
    });

    return res.json(preview);
  } catch (e: unknown) {
    console.error("[migration/billing-plans] preview", e);
...
router.post("/apply", async (req, res) => {
  try {
    ...
    const result = await applyMigrationBillingPlansImport({ schoolId, sessionId });
    return res.json(result);
  } catch (e: unknown) {
    console.error("[migration/billing-plans] apply", e);
