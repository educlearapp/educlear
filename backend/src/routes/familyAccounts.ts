...
function actorEmailFromRequest(req: { body?: Record<string, unknown>; query?: Record<string, unknown> }) {
  const fromBody = String(req.body?.actorEmail || "").trim();
  if (fromBody) return fromBody;
  return String(req.query?.actorEmail || "").trim() || undefined;
}

function pickBodyString(body: Record<string, unknown> | undefined, keys: string[]): string {
  if (!body) return "";
...
function sanitizeFamilyAccountError(error: unknown, fallback: string): string {
...
function sanitizeFamilyAccountErrorMessage(raw: string, fallback: string): string {
...
router.post("/merge", async (req, res) => {
  ...
  try {
    ...
    if (!hasSource || !hasTarget) {
      const reason =
        "Source and target are required (sourceFamilyAccountId or sourceAccountRef or sourceLearnerId, and matching target fields)";
...
router.post("/unmerge", async (req, res) => {
  try {
    ...
    const createNewAccount =
      req.body?.createNewAccount !== undefined
    ...
    const result = await unmergeLearnerFromFamily({
      schoolId,
      learnerId,
      createNewAccount,
      actorEmail: actorEmailFromRequest(req),
...
router.get("/audit", async (req, res) => {
  try {
    ...
    const entries = listFamilyAccountAudit(schoolId, limit);
