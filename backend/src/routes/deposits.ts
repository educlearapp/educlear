...
function formatDepositDate(value: Date): string {
...
function computeStatus(
  ...
  if (current === "REFUNDED" || current === "VOID") return current;
  if (remainingBalance <= 0) return "FULLY_ALLOCATED";
  if (remainingBalance < amount) return "PARTIALLY_ALLOCATED";
  return "ACTIVE";
}

async function nextDepositNumber(schoolId: string): Promise<string> {
  const count = await prisma.billingDeposit.count({ where: { schoolId } });
  return `DEP-${String(count + 1).padStart(5, "0")}`;
}

type DepositWithRelations = Prisma.BillingDepositGetPayload<{
  include: {
    learner: { select: { id: true; firstName: true; lastName: true; familyAccount: { select: { accountRef: true; familyName: true } } } };
...
function mapDepositRow(deposit: DepositWithRelations) {
  const accountNo =
    deposit.familyAccount?.accountRef ||
    deposit.learner.familyAccount?.accountRef ||
    "";
  const accountLabel =
    deposit.familyAccount?.familyName ||
...
function buildOpenInvoices(schoolId: string, learnerId: string, accountNo: string) {
  const ledger = readSchoolLedger(schoolId);
  const invoices = listInvoices(schoolId).filter(
    (e) => e.learnerId === learnerId && (!accountNo || e.accountNo === accountNo)
  );
...
router.get("/", async (req, res) => {
  try {
    ...
    const where: Prisma.BillingDepositWhereInput = { schoolId };
    if (statusFilter) {
      where.status = statusFilter as DepositStatus;
    }

    const deposits = await prisma.billingDeposit.findMany({
      where,
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const existing = await prisma.billingDeposit.findFirst({
      where: { id, schoolId },
    ...
    const reference =
      body.reference !== undefined ? String(body.reference || "").trim() || null : undefined;
    ...
    const updates: Prisma.BillingDepositUpdateInput = {};
    ...
    if (allocationRows.length > 0) {
      ...
      for (const row of allocationRows) {
        const ledgerInvoiceId = String((row as any)?.ledgerInvoiceId || (row as any)?.invoiceId || "").trim();
        const allocateAmount = normaliseAmount((row as any)?.amount);
        if (!ledgerInvoiceId || allocateAmount <= 0) continue;
      ...
      for (const row of allocationRows) {
        ...
        await prisma.billingDepositAllocation.create({
          data: {
            schoolId,
            depositId: id,
            ledgerInvoiceId,
        ...
        await prisma.billingDepositHistoryEntry.create({
          data: {
            schoolId,
            depositId: id,
            action: "ALLOCATED",
    ...
    let nextStatus: DepositStatus = existing.status;
    if (manualStatus === "REFUNDED" || manualStatus === "VOID") {
      nextStatus = manualStatus as DepositStatus;
    } else {
      nextStatus = computeStatus(existing.amount, remainingBalance, existing.status);
    }
    updates.status = nextStatus;

    const deposit = await prisma.billingDeposit.update({
      where: { id },
      data: updates,
      include: depositInclude,
    });

    const accountNo = resolveLearnerAccountNo({
