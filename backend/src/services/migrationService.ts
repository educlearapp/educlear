...
export async function commitMigrationImport(_opts: {
  schoolId: string;
  projectId: string;
  confirmToken: string;
}): Promise<{
  ...
  throw new Error(
    "LEGACY_MIGRATION_IMPORT_DISABLED: POST /api/super-admin/migration/import is disabled. " +
      "It bypassed the Parent Identity Resolver and could create duplicate Parent rows. " +
      "Use Universal Migration: POST /api/migration/apply (with FULL_MIGRATION_PREFLIGHT first, then APPLY after parent review)."
  );
}
