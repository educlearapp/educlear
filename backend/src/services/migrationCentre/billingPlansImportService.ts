...
export async function previewMigrationBillingPlansImport(opts: {
  ...
  // IMPORTANT: Preview must not call readSchoolBillingPlansResolved because that can auto-migrate JSON to DB.



  const existingPlanLearners = await countSchoolBillingPlansInDb(schoolId);
  ...
  console.log(
    ...
    `[billingPlansPreview] schoolId=${schoolId} matched=${matchedCount} unmatched=${unmatchedCount} wouldSave=${plansByLearnerId.size} noDbWrite=true`
...
export async function applyMigrationBillingPlansImport(opts: {
  ...
  const safePlansByLearnerId: Record<string, StoredBillingPlanItem[]> = {};



  for (const [learnerId, items] of Object.entries(payload.plansByLearnerId || {})) {
    ...
    if (existingLearnerIds.has(learnerId)) {
      ...
      safePlansByLearnerId[learnerId] = items;
  ...
  const skippedMissingLearners = planKeys.length - Object.keys(safePlansByLearnerId).length;



  if (!Object.keys(safePlansByLearnerId).length) {



    throw new Error("No valid matched learner billing plans to apply");
  ...
  const savedCount = await upsertSchoolBillingPlansToDb(schoolId, safePlansByLearnerId);
  ...
  console.log(
    ...
    `[billingPlansApply] schoolId=${schoolId} validRows=${Object.keys(safePlansByLearnerId).length} skippedMissingLearners=${skippedMissingLearners} savedCount=${savedCount}`
