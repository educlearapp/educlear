...
function bumpCount(counts: MigrationApplyCounts, entity: MigrationImportEntityType): void {
  switch (entity) {
    case "learner":
      counts.learners += 1;
      break;
...
function createdRecordIds(
  ...
  for (const row of rows) {
    if (row.status !== "created" || row.entityType !== entityType) continue;
...
export function batchHasCreatedTransactions(reportRows: MigrationImportReportRow[]): boolean {
...
export async function rollbackMigrationBatch(
  ...
  if (!batchId) throw new MigrationRollbackError("batchId is required");
  if (!targetSchoolId) throw new MigrationRollbackError("targetSchoolId is required");
  if (!confirmationText) throw new MigrationRollbackError("confirmationText is required");

  const batch = getImportBatch(batchId);
  if (!batch) throw new MigrationRollbackError("Import batch not found");

  if (batch.status !== "completed" && batch.status !== "failed") {
    throw new MigrationRollbackError(
      `Rollback is only allowed for completed or failed batches (current status: ${batch.status})`
    );
  }
  ...
  if (batchHasCreatedTransactions(reportRows)) {
    throw new MigrationRollbackError(
      "This batch contains posted ledger transactions. Use reversal rollback, not deletion rollback."
    );
  }
  ...
  const rollbackReport: MigrationRollbackReportRow[] = [];

  const pushRollback = (row: MigrationRollbackReportRow) => {
    rollbackReport.push(row);
    if (row.status === "deleted") bumpCount(deletedCounts, row.entityType);
    else bumpCount(blockedCounts, row.entityType);
  };
