...
function countReportRows(
  rows: MigrationImportReportRow[],
  entityType: MigrationImportEntityType,
  statuses?: Set<MigrationImportReportRow["status"]>
): number {
  return rows.filter((row) => {
    if (row.entityType !== entityType) return false;
    if (statuses && !statuses.has(row.status)) return false;
    return true;
  }).length;
}

function sumReportByStatus(
  rows: MigrationImportReportRow[],
  entityType: MigrationImportEntityType
): Record<MigrationImportReportRow["status"], number> {
  const out: Record<MigrationImportReportRow["status"], number> = {
    created: 0,
    skipped: 0,
    failed: 0,
    not_applied: 0,
...
function summarizeReportEntity(
  rows: MigrationImportReportRow[],
  entityType: MigrationImportEntityType
): string {
...
function isMigrationPostedEntry(entry: BillingLedgerEntry): boolean {
  const source = cleanString(entry.source);
  if (source === MIGRATION_LEDGER_SOURCE) return true;
  return entry.id.startsWith("umig-tx-");
}

function isMigrationReversalEntry(entry: BillingLedgerEntry, batchId: string): boolean {
  const source = cleanString(entry.source);
  if (source === REVERSAL_SOURCE) return true;
  const safeBatch = batchId.replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 24);
  return entry.id.startsWith(`umig-rev-${safeBatch}-`);
}

function ledgerSignedAmount(entry: BillingLedgerEntry): number {
  const amount = normaliseAmount(entry.amount);
  if (entry.type === "payment" || entry.type === "credit") return -amount;
  return amount;
}

function buildSummary(checks: MigrationReconciliationCheck[]): MigrationReconciliationSummary {
  let passed = 0;
  let warnings = 0;
  let failed = 0;
...
function overallStatus(summary: MigrationReconciliationSummary): MigrationReconciliationStatus {
...
function pushCheck(
  ...
  partial: Omit<MigrationReconciliationCheck, "id"> & { id?: string }
): void {
  checks.push({
...
export async function reconcileMigrationBatch(
  ...
  if (!batchId) throw new MigrationReconciliationError("batchId is required");
  if (!targetSchoolId) throw new MigrationReconciliationError("targetSchoolId is required");
  ...
  const learnerReportTotal =
    countReportRows(reportRows, "learner") ||
    createdCounts.learners + skippedCounts.learners + failedCounts.learners;
  const parentReportTotal =
    countReportRows(reportRows, "parent") ||
    createdCounts.parents + skippedCounts.parents + failedCounts.parents;
  const billingReportTotal =
    countReportRows(reportRows, "billingAccount") ||
  ...
  pushCheck(checks, {
    ...
    message:
      stagedCounts.learners === learnerRowCount
  ...
  pushCheck(checks, {
    ...
    status:
      stagedCounts.parents === countReportRows(reportRows, "parent") ? "pass" : "warning",
    message:
  ...
  pushCheck(checks, {
    ...
    actual:
      summarizeReportEntity(reportRows, "billingAccount") || String(billingReportTotal),
    status:
      stagedCounts.billingAccounts === countReportRows(reportRows, "billingAccount")
        ? "pass"
        : "warning",
    message:
      stagedCounts.billingAccounts === countReportRows(reportRows, "billingAccount")
  ...
  pushCheck(checks, {
    ...
    status:
      createdCounts.learners === countReportRows(reportRows, "learner", new Set(["created"])) &&
  ...
  pushCheck(checks, {
    ...
    status:
      createdCounts.parents === countReportRows(reportRows, "parent", new Set(["created"])) &&
  ...
  if (createdLearnerIds.length > 0) {
    const createdLearners = await prisma.learner.findMany({
      where: { schoolId: targetSchoolId, id: { in: createdLearnerIds } },
      select: { id: true, enrollmentStatus: true },
    });
    for (const l of createdLearners) {
      if (l.enrollmentStatus === "ACTIVE") batchCreatedStillActive += 1;
      else batchCreatedNonActive += 1;
    }
  }

  const impliedBeforeActive =
  ...
  pushCheck(checks, {
    ...
    status:
      batch.status === "completed" && expectedAfterActive === actualActiveCount
        ? "pass"
        : batch.status === "rolled_back"
          ? "warning"
  ...
  pushCheck(checks, {
    ...
    status:
      batchCreatedNonActive === 0 && unenrolledOrOtherCount === 0
        ? "pass"
  ...
  const createdTransactionRows = reportRows.filter(
    (r) => r.entityType === "transaction" && r.status === "created" && cleanString(r.recordId)
  );
  const expectedPostedCount = createdTransactionRows.length;
  const ledgerMigrationIds = new Set(migrationPosted.map((e) => e.id));
  const missingLedger = createdTransactionRows.filter(
    (r) => !ledgerMigrationIds.has(cleanString(r.recordId))
  );
  ...
  pushCheck(checks, {
    ...
    status:
      expectedPostedCount === migrationPosted.length && missingLedger.length === 0
        ? "pass"
  ...
  pushCheck(checks, {
    ...
    actual: `migration net R${migrationNet.toFixed(2)} (${migrationPosted.length} entries)${
      batch.status === "rolled_back"
    ...
    status: "pass",
    message:
      "Read-only summary of signed amounts on billing-ledger.json (invoices/penalties positive, payments/credits negative).",
  });

  const blockedUnmatched = reportRows.filter(
    (r) =>
  ...
  pushCheck(checks, {
    ...
    status:
      stagedTransactions === transactionReportTotal ? "pass" : "warning",
    message:
      stagedTransactions === transactionReportTotal
