import type {
  MigrationPilotBuildInput,
  MigrationPilotBuildResult,
  MigrationPilotDryRunSummary,
  MigrationPilotReconciliationSummary,
  MigrationPilotStatus,
  MigrationPilotUploadedFile,
  MigrationPilotValidationSummary,
  MigrationPilotVerificationCheck,
} from "../types/MigrationPilot";
...
export class MigrationPilotError extends Error {
...
function emptyValidationSummary(): MigrationValidationSummary {
  return {
    mode: "preview",
    rowsChecked: 0,
    totalIssues: 0,
...
function defaultDryRunSummary(sourceSystem: string): MigrationPilotDryRunSummary {
  return {
    stageCreated: false,
    sourceSystem,
    canApply: false,
...
function defaultReconciliationSummary(): MigrationPilotReconciliationSummary {
  return {
    run: false,
    passed: 0,
    warnings: 0,
...
function hydrateValidationSummary(
  input: MigrationPilotBuildInput,
  partial?: Partial<MigrationPilotValidationSummary>
): MigrationPilotValidationSummary {
  const base = partial ?? input.validationSummary ?? {};
  const stage = input.stageId ? getStage(input.stageId) : null;
  const fromStage = stage?.validationSummary;

  const merged: MigrationValidationSummary = {
    ...emptyValidationSummary(),
    ...(fromStage ?? {}),
    ...(base as MigrationValidationSummary),
  };
...
function hydrateDryRunSummary(input: MigrationPilotBuildInput): MigrationPilotDryRunSummary {
  ...
  const headCountProtected =
    validationErrors === 0 &&
    stage.canApply &&
    blocked === 0 &&
    (stage.stagedCounts.historical ?? 0) >= 0;

  const historicalLearnersProtected =
    (stage.stagedCounts.historical ?? 0) > 0 ||
...
async function hydrateReconciliationSummary(
  input: MigrationPilotBuildInput
): Promise<MigrationPilotReconciliationSummary> {
  ...
  if (!batch) throw new MigrationPilotError("Import batch not found");
  ...
  let reconciliation;
  try {
    reconciliation = await reconcileMigrationBatch({ batchId, targetSchoolId: schoolId });
  } catch (e: unknown) {
  ...
  const historicalCheck = reconciliation.checks.find(
    (c) => c.id === "historical_excluded_from_head_count"
  );
...
export function hasPilotCriticalFailures(input: {
  ...
  if (input.validationSummary.errors > 0) return true;
  ...
  if (
    input.reconciliationSummary.run &&
...
export function isDryRunClean(dryRunSummary: MigrationPilotDryRunSummary): boolean {
  if (!dryRunSummary.stageCreated) return false;
  if (!dryRunSummary.canApply) return false;
  if (dryRunSummary.validationErrors > 0) return false;
  if (dryRunSummary.transactionReadiness.blockedTransactions > 0) return false;
  return true;
}

export function computeMigrationPilotStatus(input: {
  validationSummary: MigrationPilotValidationSummary;
  dryRunSummary: MigrationPilotDryRunSummary;
  reconciliationSummary: MigrationPilotReconciliationSummary;
}): MigrationPilotStatus {
  ...
  if (!recon.run) {
    ...
    if (!dryRunClean) return "warning";
    if (input.validationSummary.mode !== "full") return "draft";
    return "draft";
  }

  if (critical || recon.overallStatus === "fail") return "failed";

  if (
    recon.overallStatus === "pass" &&
  ...
  if (recon.overallStatus === "pass" && dryRunClean) return "passed";
...
function collectStatusReasons(input: {
  ...
  if (dryRunSummary.transactionReadiness.blockedTransactions > 0) {
    reasons.push(
      `${dryRunSummary.transactionReadiness.blockedTransactions} blocked transaction(s) in readiness review.`
    );
  }
  if (!reconciliationSummary.run) {
    ...
    reasons.push(
      `Reconciliation overall: ${String(reconciliationSummary.overallStatus ?? "unknown").toUpperCase()}.`
    );
...
function hasSignoffForBatch(batchId: string | undefined, schoolId: string): boolean {
  if (!batchId) return false;
  const signoffs = listSignoffs();
  return signoffs.some((s) => s.batchId === batchId && s.schoolId === schoolId);
}

export function buildPilotVerificationChecks(input: {
  ...
  const fullValidation =
    input.validationSummary.mode === "full" &&
    input.validationSummary.canProceed &&
    input.validationSummary.errors === 0;
  const dryRunReviewed =
    input.dryRunSummary.stageCreated && input.dryRunSummary.validationErrors === 0;
  const transactionReadinessReviewed =
    input.dryRunSummary.stageCreated &&
  ...
  const reconciliationCompleted =
    input.reconciliationSummary.run && Boolean(input.reconciliationSummary.overallStatus);
  const signoffGenerated = hasSignoffForBatch(input.batchId, input.schoolId);

  const checks: MigrationPilotVerificationCheck[] = [
    {
    ...
    },
    {
      ...
      advisory: true,
      satisfied:
        input.dryRunSummary.historicalLearnersProtected ||
    ...
    },
    {
      ...
      advisory: true,
      satisfied:
        input.reconciliationSummary.headCountProtected || input.dryRunSummary.headCountProtected,
...
export async function buildMigrationPilot(
  input: MigrationPilotBuildInput
): Promise<MigrationPilotBuildResult> {
  const schoolId = cleanString(input.schoolId);
  const schoolName = cleanString(input.schoolName);
  const sourceSystem = cleanString(input.sourceSystem);

  if (!schoolId) throw new MigrationPilotError("schoolId is required");
  if (!schoolName) throw new MigrationPilotError("schoolName is required");
  if (!sourceSystem) throw new MigrationPilotError("sourceSystem is required");
