...
function isStaffFilename(haystack: string, basename: string): boolean {
  const base = basename.toLowerCase();
  if (haystackIncludesKeyword(haystack, "employee") || base.includes("employee")) return true;
  if (haystackIncludesKeyword(haystack, "staff") || base.includes("staff")) return true;
  if (haystackIncludesKeyword(haystack, "teacher") || base.includes("teacher")) return true;
  if (haystackIncludesKeyword(haystack, "payroll") || base.includes("payroll")) return true;
  return false;
}
 ...
 */
export function isLearnerClassExportFilename(haystack: string, basename: string): boolean {
  const base = basename.trim();
  const lower = base.toLowerCase();
  const compactBase = compactAlphanumeric(base);

  if (/^grade[_\s-]/i.test(base) || /^grade\s+\d/i.test(base)) return true;
  if (/^grade[_\s-]?r[a-z]?$/i.test(base)) return true;
  if (/^grade[_\s-]?\d+[a-z]?$/i.test(base)) return true;
  if (/^grade\d+[a-z]?$/i.test(compactBase)) return true;

  if (/^gr[_\s-]/i.test(base) || /^gr\s+\d/i.test(base)) return true;
  if (/^gr\d+[a-z]?$/i.test(compactBase)) return true;

  if (/^class[_\s-]/i.test(base) && !/class[\s_-]*list/i.test(base)) return true;
  if (/^class\s+\d/i.test(base)) return true;

  if (/^(creche|preschool|pre[_\s-]?school|reception|rrr|rr)$/i.test(lower)) return true;
  if (/^(creche|preschool|reception|rrr|rr)$/.test(haystack)) return true;
  if (haystack.includes("creche") || haystack.includes("preschool")) {
    return true;
  }
  if (haystack.includes("reception")) return true;

  if (/^grade(r\d+[a-z]?|\d+[a-z]?)$/.test(haystack)) return true;
  if (/^gr(r?\d+[a-z]?)$/.test(haystack)) return true;
