import { GENERIC_EXCEL_CONFIDENCE_RULES } from "../adapters/genericExcelMetadata";
import {
  isAmbiguousGenericExcelColumn,
  normalizeGenericExcelColumn,
} from "../adapters/genericExcelNormalization";
import { normalizeKidESysColumn } from "../adapters/kideesysNormalization";
import { SASAMS_CONFIDENCE_RULES } from "../adapters/sasamsMetadata";
import {
...
export type SuggestColumnMappingsInput = {
  fileId: string;
  filename: string;
  category: string;
  columns: string[];
...
export type SuggestColumnMappingsResult = {
  fileId: string;
  filename: string;
  category: string;
...
type KeywordRule = {
  target: MigrationTargetField;
  keywords: string[];
  baseConfidence: number;
  reason: string;
  /** Extra score when file category aligns with field group. */
...
function columnHaystack(column: string): string {
...
function scoreRule(
  ...
  if (compactColumnKey(longest).length >= 8) score += 4;
...
function kidESysMappingForColumn(
  column: string,
  category: string
): { target: MigrationTargetField; confidence: number; reason: string } | null {
  const target = normalizeKidESysColumn(column, category);
...
function genericExcelMappingForColumn(
  column: string
): { target: MigrationTargetField; confidence: number; reason: string } | null {
  const target = normalizeGenericExcelColumn(column);
  if (!target) return null;
  const ambiguous = isAmbiguousGenericExcelColumn(column);
...
function sasamsMappingForColumn(
  column: string
): { target: MigrationTargetField; confidence: number; reason: string } | null {
  const target = normalizeSASAMSColumn(column);
  if (!target) return null;
  const ambiguous = isAmbiguousSASAMSColumn(column);
...
function bestMappingForColumn(
  ...
  if (!best || best.confidence < MIN_SUGGESTION_CONFIDENCE) return null;
  return best;
}
...
export { MAPPED_CONFIDENCE_THRESHOLD };
