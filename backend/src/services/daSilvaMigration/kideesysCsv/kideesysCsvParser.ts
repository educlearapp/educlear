...
export const KIDEESYS_CSV_TYPES = [
  "accounts",
  "child",
  "child_parent",
  "invoices",
...
export type KidESysCsvType = (typeof KIDEESYS_CSV_TYPES)[number];

export type KidESysCsvExpectedFile = `${KidESysCsvType}.csv`;

export const KIDEESYS_CSV_FILE_SUFFIX: Record<KidESysCsvType, KidESysCsvExpectedFile> = {
...
export type ParsedKidESysChild = {
  childId: string;
  accountNo: string;
  firstName: string;
  lastName: string;
...
export type ParsedKidESysChildParent = {
  childId: string;
  parentId: string;
  relationship: string;
...
export type ParsedKidESysAccount = {
  accountId: string;
  accountNo: string;
  familyName: string;
  balance: number;
...
export type ParsedKidESysMonthlyAccount = {
  childId: string;
  accountNo: string;
...
export type ParsedKidESysInvoice = {
  invoiceId: string;
  accountNo: string;
  childId: string;
  fullName: string;
...
export type ParsedKidESysPayment = {
  paymentId: string;
  accountNo: string;
  childId: string;
  fullName: string;
...
export type ParsedKidESysJournal = {
  journalId: string;
  accountNo: string;
  childId: string;
  fullName: string;
...
export type KidESysCsvBundle = {
  sourcePath: string;
  csvDir: string;
  filesFound: Record<KidESysCsvType, string>;
  headersByFile: Record<KidESysCsvType, string[]>;
  children: ParsedKidESysChild[];
  childParents: ParsedKidESysChildParent[];
  accounts: ParsedKidESysAccount[];
  monthlyAccounts: ParsedKidESysMonthlyAccount[];
  invoices: ParsedKidESysInvoice[];
  payments: ParsedKidESysPayment[];
  journals: ParsedKidESysJournal[];
};

export const KIDEESYS_CSV_REQUIRED_COLUMNS: Record<KidESysCsvType, string[][]> = {
...
function rowLookup(row: Record<string, string>): Map<string, string> {
...
export function pickCsvField(row: Record<string, string>, aliases: string[]): string {
...
function csvBasenameMatchesType(filename: string, csvType: KidESysCsvType): boolean {
  const lower = path.basename(filename).toLowerCase();
  const suffix = KIDEESYS_CSV_FILE_SUFFIX[csvType];
  if (lower === suffix) return true;
  if (lower.endsWith(`_${suffix}`)) return true;
  return lower.endsWith(suffix);
}

export function findKidESysCsvFiles(rootDir: string): Partial<Record<KidESysCsvType, string>> {
  const found: Partial<Record<KidESysCsvType, string>> = {};
...
export function assertKidESysCsvBundleComplete(
  files: Partial<Record<KidESysCsvType, string>>
): asserts files is Record<KidESysCsvType, string> {
  const missing = KIDEESYS_CSV_TYPES.filter((name) => !files[name]);
  if (missing.length) {
    const friendly = missing.map((t) => KIDEESYS_CSV_FILE_SUFFIX[t]).join(", ");
    throw new Error(
      `Kid-e-Sys CSV export is missing required file(s): ${friendly}. ` +
...
export function validateRequiredColumns(
  csvType: KidESysCsvType,
  headers: string[]
): string[] {
  const headerSet = new Set(headers.map(normalizeHeaderKey));
  const missing: string[] = [];
  for (const group of KIDEESYS_CSV_REQUIRED_COLUMNS[csvType]) {
...
export function extractKidESysCsvZip(zipPath: string): string {
  const resolved = path.resolve(zipPath);
  if (!fs.existsSync(resolved)) throw new Error(`ZIP not found: ${resolved}`);
  const dest = fs.mkdtempSync(path.join(os.tmpdir(), "kideesys-csv-"));
  execSync(`unzip -q -o ${JSON.stringify(resolved)} -d ${JSON.stringify(dest)}`, {
    stdio: "pipe",
  });
  return dest;
}

export function resolveKidESysCsvDirectory(sourcePath: string): {
  csvDir: string;
  cleanup: () => void;
} {
  const resolved = path.resolve(sourcePath);
  if (!fs.existsSync(resolved)) throw new Error(`Source not found: ${resolved}`);
...
function buildChildMatchKey(fullName: string, className: string): string {
...
function parseChildRows(rows: Record<string, string>[]): ParsedKidESysChild[] {
  const out: ParsedKidESysChild[] = [];
  for (const row of rows) {
    ...
    if (!childId && !fullName && !resolvedFirst) continue;
...
function parseAccountRows(rows: Record<string, string>[]): ParsedKidESysAccount[] {
  const out: ParsedKidESysAccount[] = [];
  for (const row of rows) {
    ...
    if (!accountNo && !accountId) continue;
    const familyName =
      pickCsvField(row, [
        "family_name",
        "surname",
    ...
    out.push({
      ...
      contactFirstName:
        pickCsvField(row, ["first_name", "contact_first_name", "child_name"]) ||
        contactParts.firstName,
      contactSurname:
        pickCsvField(row, ["last_name", "contact_surname", "child_surname"]) ||
...
function parseChildParentRows(rows: Record<string, string>[]): ParsedKidESysChildParent[] {
  const out: ParsedKidESysChildParent[] = [];
  for (const row of rows) {
    ...
    const relationship =
      pickCsvField(row, ["relationship", "relation", "parent_type", "type"]) || "Guardian";
    out.push({
      childId,
      parentId,
      relationship,
      parentFirstName:
        pickCsvField(row, ["name", "first_name", "parent_first_name"]) || nameParts.firstName,
      parentSurname:
        pickCsvField(row, ["surname", "last_name", "parent_surname"]) || nameParts.lastName,
...
function parseMonthlyAccountRows(rows: Record<string, string>[]): ParsedKidESysMonthlyAccount[] {
  const out: ParsedKidESysMonthlyAccount[] = [];
  for (const row of rows) {
    ...
    if (!feeDescription || !amount) continue;
    out.push({
...
function dedupeInvoiceRows(rows: ParsedKidESysInvoice[]): ParsedKidESysInvoice[] {
  const byKey = new Map<string, ParsedKidESysInvoice>();
...
function parseInvoiceRows(rows: Record<string, string>[]): ParsedKidESysInvoice[] {
  const out: ParsedKidESysInvoice[] = [];
  for (const row of rows) {
    const invoiceId =
      pickCsvField(row, [
        "transaction_id",
    ...
    if (!invoiceId && !accountNo) continue;
    if (!amount) continue;
    out.push({
...
function dedupePaymentRows(rows: ParsedKidESysPayment[]): ParsedKidESysPayment[] {
  const byKey = new Map<string, ParsedKidESysPayment>();
  for (const pay of rows) {
    const key = `${pay.accountNo}|${pay.paymentId}`;
    if (!byKey.has(key)) byKey.set(key, pay);
  }
  return Array.from(byKey.values());
}

function parsePaymentRows(rows: Record<string, string>[]): ParsedKidESysPayment[] {
  const out: ParsedKidESysPayment[] = [];
  for (const row of rows) {
    const paymentId =
      pickCsvField(row, ["transaction_id", "payment_no", "id", "payment_id", "receipt_no", "receipt_number", "number"]) ||
      "";
    const accountNo = pickCsvField(row, ["account_no", "account_number", "account_ref", "account_id"]);
    const amountRaw = pickCsvField(row, ["amount", "payment_amount", "total", "value", "detail_amount"]);
    const amount = Math.abs(Number(String(amountRaw || "0").replace(/,/g, "")) || 0);
    if (!paymentId && !accountNo) continue;
    if (!amount) continue;
    out.push({
...
function dedupeJournalRows(rows: ParsedKidESysJournal[]): ParsedKidESysJournal[] {
  const byKey = new Map<string, ParsedKidESysJournal>();
  for (const journal of rows) {
    const key = `${journal.accountNo}|${journal.journalId}`;
    if (!byKey.has(key)) byKey.set(key, journal);
  }
  return Array.from(byKey.values());
}
...
function classifyJournal(
  ...
  creditRaw: string,
  amountRaw: string
): { kind: "invoice" | "payment" | "credit"; signedAmount: number; amount: number } {
  ...
  if (amount > 0) {
    ...
    if (signed < 0) return { kind: "payment", signedAmount: signed, amount: Math.abs(signed) };
    return { kind: "invoice", signedAmount: signed, amount: Math.abs(signed) };
  }
  return { kind: "credit", signedAmount: 0, amount: 0 };
}

function parseJournalRows(rows: Record<string, string>[]): ParsedKidESysJournal[] {
  const out: ParsedKidESysJournal[] = [];
  for (const row of rows) {
    const journalId =
      pickCsvField(row, ["transaction_id", "journal_no", "id", "journal_id", "entry_id"]) || "";
    const accountNo = pickCsvField(row, ["account_no", "account_number", "account_ref", "account_id"]);
    ...
    if (!journalId && !accountNo) continue;
    if (!classified.amount) continue;
    out.push({
...
export function loadKidESysCsvBundle(sourcePath: string): KidESysCsvBundle {
  ...
  try {
    ...
    const headersByFile = {} as Record<KidESysCsvType, string[]>;
    for (const csvType of KIDEESYS_CSV_TYPES) {
