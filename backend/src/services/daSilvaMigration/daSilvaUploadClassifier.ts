...
export type DaSilvaUploadSlotKind =
  | "classList"
  | "learnerRegister"
  | "parentLearnerLinks"
  | "parentRegister"
...
const FIELD_TO_SLOT: Record<string, DaSilvaUploadSlotKind> = {
...
function isClassListField(field: string): boolean {
  const n = normalizeField(field);
  if (CLASS_LIST_FIELDS.has(n)) return true;
  return /^classlistfiles/i.test(field.trim());
}

function lowerName(originalName: string): string {
  return path.basename(String(originalName || "")).toLowerCase();
}

function isSpreadsheet(originalName: string): boolean {
  return /\.xlsx?$/i.test(originalName);
}

function classifyByFilename(originalName: string): DaSilvaUploadSlotKind {
  const name = lowerName(originalName);
  if (!name || !isSpreadsheet(originalName)) return "unknown";
...
export function classifyDaSilvaUploadFile(
  fieldname: string,
  originalname: string
): DaSilvaUploadSlotKind {
  const field = String(fieldname || "").trim();
  if (isClassListField(field)) return "classList";

  const normalized = normalizeField(field);
  if (FIELD_TO_SLOT[normalized]) return FIELD_TO_SLOT[normalized];

  return classifyByFilename(originalname);
}

export function canonicalRelativePathForSlot(kind: DaSilvaUploadSlotKind, originalname: string): string | null {
  switch (kind) {
    case "classList":
      return `sasams/class_lists/${path.basename(originalname)}`;
