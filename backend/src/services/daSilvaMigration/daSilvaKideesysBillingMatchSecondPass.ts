...
import type {
  BillingAccountMatchRow,
  BillingMatchAudit,
  DbLearnerForBillingMatch,
...
export type BillingMatchConfidence = "high" | "medium" | "low";

export type BillingMatchCandidate = {
  learnerId: string;
  fullName: string;
  className: string | null;
  strategy: string;
  confidence: BillingMatchConfidence;
};

export type UnmatchedBillingReconciliationRow = {
  accountNo: string;
  learnerName: string;
  siblingFamilyNames: string[];
  balance: number;
  matchedCandidates: BillingMatchCandidate[];
  reasonNotMatched: string;
  secondPassLearnerId: string | null;
  secondPassStrategy: string | null;
  secondPassConfidence: BillingMatchConfidence | null;
  disposition: "auto_matched" | "manual_review" | "still_unmatched";
};

export type KideesysBillingReconciliationReport = {
  generatedAt: string;
  ...
  autoMatched: UnmatchedBillingReconciliationRow[];
  manualReviewRequired: UnmatchedBillingReconciliationRow[];
  stillUnmatchedRows: UnmatchedBillingReconciliationRow[];
  reconciliationRows: UnmatchedBillingReconciliationRow[];
};

export type SecondPassBillingMatchInput = {
  accounts: ParsedBillingAccount[];
  dbLearners: DbLearnerForBillingMatch[];
  classListLearners: ParsedLearner[];
  billingPlanItems?: ParsedBillingPlanItem[];
...
function firstNameVariants(first: string): string[] {
  ...
  for (const alias of aliases) variants.add(normName(alias));
...
type LearnerIndexes = {
  byId: Map<string, DbLearnerForBillingMatch>;
  byMatchKey: Map<string, string[]>;
  byNameOnly: Map<string, string[]>;
  byNormSurname: Map<string, string[]>;
  bySurnameClass: Map<string, string[]>;
  byContactKey: Map<string, string[]>;
  byBillingPlanKey: Map<string, string[]>;
  byTxnName: Map<string, string[]>;
};
...
function buildLearnerIndexes(
  learners: DbLearnerForBillingMatch[],
  contacts: ParsedLearnerContact[],
  billingPlanItems: ParsedBillingPlanItem[],
  transactions: ParsedTransaction[]
): LearnerIndexes {
  const byId = new Map<string, DbLearnerForBillingMatch>();
  const byMatchKey = new Map<string, string[]>();
  const byNameOnly = new Map<string, string[]>();
  const byNormSurname = new Map<string, string[]>();
  const bySurnameClass = new Map<string, string[]>();
  const byContactKey = new Map<string, string[]>();
  const byBillingPlanKey = new Map<string, string[]>();
  const byTxnName = new Map<string, string[]>();
  ...
  for (const l of learners) {
    ...
    pushIndex(
      byNameOnly,
      `${normName(l.lastName)}|${normName(l.firstName)}`,
      l.id
    );
  ...
  for (const contact of contacts) {
    ...
    const ids =
      learners
        .filter(
          (l) =>
            normName(`${l.firstName} ${l.lastName}`) === key ||
            (normName(l.firstName) === normName(contact.firstName) &&
              normName(l.lastName) === normName(contact.lastName))
        )
        .map((l) => l.id) || [];
    if (ids.length) pushIndex(byContactKey, key, ids[0]);
    for (const id of ids) pushIndex(byContactKey, key, id);
  }
  ...
  for (const item of billingPlanItems) {
    const ids = learners
      ...
      .map((l) => l.id);
    if (ids.length) pushIndex(byBillingPlanKey, item.matchKey, ids[0]);
    for (const id of ids) pushIndex(byBillingPlanKey, item.matchKey, id);
  }
  ...
  for (const txn of transactions) {
    ...
    const ids = learners
      ...
      .map((l) => l.id);
    for (const id of ids) pushIndex(byTxnName, key, id);
  }
...
function toCandidate(
  ...
  strategy: string,
  confidence: BillingMatchConfidence,
  indexes: LearnerIndexes
): BillingMatchCandidate {
...
function collectCandidatesForName(
  displayName: string,
  ...
  const candidates: BillingMatchCandidate[] = [];
  const seen = new Set<string>();

  const add = (ids: string[], strategy: string, confidence: BillingMatchConfidence) => {
    for (const id of uniqueIds(ids)) {
      if (alreadyMatchedIds.has(id) || seen.has(id)) continue;
      seen.add(id);
  ...
  const planKey = buildLearnerMatchKey(`${firstName} ${lastName}`, className);
  ...
  add(
    txnNameHits,
    "transaction_name",
    uniqueIds(txnNameHits).length === 1 ? "high" : "medium"
  );

  const accountFromFamily = findAccountForLearnerName(displayName, [account], familyIndex);
  ...
  for (const item of billingPlanItems) {
    if (item.matchKey !== planKey) continue;
    add(
      indexes.byBillingPlanKey.get(item.matchKey) || [],
      "billing_plan_crossref",
      "high"
    );
  }

  const billingTokens = new Set(
    cleanBillingDisplayName(displayName)
      .split(/\s+/)
      .map((t) => normName(t))
      .filter(Boolean)
  );
  if (billingTokens.size >= 2) {
    ...
    add(
      tokenHits.map((l) => l.id),
      "full_name_token_overlap",
      tokenHits.length === 1 ? "high" : "low"
    );
  }

  const deduped: BillingMatchCandidate[] = [];
...
function trySiblingFamilyGroupMatch(
  ...
  transactions: ParsedTransaction[],
  alreadyMatchedIds: Set<string>
): { learnerId: string | null; strategy: string | null; candidates: BillingMatchCandidate[] } {
  if (displayNames.length < 2) {
    return { learnerId: null, strategy: null, candidates: [] };
  }

  const perName: Array<{ name: string; pick: BillingMatchCandidate | null; candidates: BillingMatchCandidate[] }> =
    [];
  for (const name of displayNames) {
    const candidates = collectCandidatesForName(
      name,
      classHint,
      account,
      indexes,
...
function reasonFromCandidates(
  candidates: BillingMatchCandidate[],
  firstPassReason: string | null
): string {
...
export function buildFamilyIndexFromAccounts(accounts: ParsedBillingAccount[]): FamilyAccountIndex {
...
export function applySecondPassBillingMatch(
  firstPass: BillingMatchAudit,
  input: SecondPassBillingMatchInput,
  accountBalances: Map<string, number>
): {
  audit: BillingMatchAudit;
  ...
  const indexes = buildLearnerIndexes(
    input.dbLearners,
    contacts,
    billingPlanItems,
    transactions
  ...
  const matchedLearnerIds = new Set(
    firstPass.matched.filter((r) => r.learnerId).map((r) => r.learnerId as string)
  );
  const matched: BillingAccountMatchRow[] = [...firstPass.matched];
  const duplicateMatches: BillingAccountMatchRow[] = [...firstPass.duplicateMatches];
  const reconciliationRows: UnmatchedBillingReconciliationRow[] = [];
  const autoMatched: UnmatchedBillingReconciliationRow[] = [];
  const manualReviewRequired: UnmatchedBillingReconciliationRow[] = [];
  const stillUnmatchedRows: UnmatchedBillingReconciliationRow[] = [];
  ...
  for (const row of firstPass.unmatchedAccounts) {
    ...
    let candidates = names.flatMap((name) =>
      collectCandidatesForName(
        name,
        classHint,
        account || {
    ...
    let secondPassLearnerId: string | null = null;
    let secondPassStrategy: string | null = null;
    let secondPassConfidence: BillingMatchConfidence | null = null;

    const siblingTry = trySiblingFamilyGroupMatch(
      names,
      classHint,
      account || {
        accountNo: row.accountNo,
    ...
    if (!secondPassLearnerId) {
      for (const name of names) {
        const nameCandidates = collectCandidatesForName(
          name,
          classHint,
          account || {
            accountNo: row.accountNo,
    ...
    if (!secondPassLearnerId && names.length > 1) {
      const resolved: BillingMatchCandidate[] = [];
      for (const name of names) {
        const nameCandidates = collectCandidatesForName(
          name,
          classHint,
    ...
    let disposition: UnmatchedBillingReconciliationRow["disposition"] = "still_unmatched";
    if (secondPassLearnerId && secondPassConfidence === "high") {
    ...
    const reconRow: UnmatchedBillingReconciliationRow = {
      accountNo: row.accountNo,
    ...
    if (disposition === "auto_matched" && secondPassLearnerId) {
      const mergedRow: BillingAccountMatchRow = {
        ...row,
  ...
  const report: KideesysBillingReconciliationReport = {
