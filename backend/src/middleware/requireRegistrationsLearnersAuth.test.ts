/**
 * Registrations learners auth decision tests (pure).
 * Run: npx tsx src/middleware/requireRegistrationsLearnersAuth.test.ts
 */
import assert from "assert";
import { evaluateRegistrationsLearnersAuth } from "./requireRegistrationsLearnersAuth";
...
function activeUser(schoolId: string) {
  return { id: "user-reg-1", schoolId, role: "ADMIN", isActive: true };
}

function testUnauthenticated() {
  const d = evaluateRegistrationsLearnersAuth({
    jwtPayload: null,
    user: null,
    appRole: "Finance",
    permissions: null,
...
function testCustomNoPermissionDenied() {
  ...
  // Custom with empty map
  const d = evaluateRegistrationsLearnersAuth({
    jwtPayload: jwtPayload(DA_SILVA),
    user: { id: "user-reg-1", schoolId: DA_SILVA, role: "CUSTOM", isActive: true },
    appRole: "Custom",
    permissions: empty,
    requestSchoolId: DA_SILVA,
  });
...
function testTeacherReportsViewAllowed() {
  const d = evaluateRegistrationsLearnersAuth({
    jwtPayload: jwtPayload(DA_SILVA),
    user: { id: "user-reg-1", schoolId: DA_SILVA, role: "TEACHER", isActive: true },
    appRole: "Teacher",
...
testFinanceAllowed();
testTeacherReportsViewAllowed();
testAuthorizedSchoolWinsWithoutQuerySchool();
console.log("\nAll requireRegistrationsLearnersAuth tests passed.");
