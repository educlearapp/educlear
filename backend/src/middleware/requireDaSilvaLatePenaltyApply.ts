import type { NextFunction, Request, Response } from "express";

import { prisma } from "../prisma";
import { isDaSilvaLatePenaltySchoolAllowed } from "../services/daSilvaLatePenaltyEngine";
import { getUserAccessMeta } from "../utils/userAccessStore";
import { appRoleFromPrismaRole } from "../utils/userPermissions";
import { verifyStaffJwt, type StaffJwtPayload } from "../utils/staffJwt";

const ALLOWED_APPLY_ROLES = new Set(["Owner", "Admin", "Finance"]);

export type DaSilvaLatePenaltyApplyRequest = Request & {
  daSilvaApplyAuth?: StaffJwtPayload & { appRole: string };
};

export type DaSilvaApplyAuthDecision =
  | { allowed: true; schoolId: string; appRole: string; auth: StaffJwtPayload & { appRole: string } }
  | { allowed: false; status: 401 | 403; error: string };

/** Pure auth decision — used by middleware and route security tests. */
export function evaluateDaSilvaLatePenaltyApplyAuth(input: {
  jwtPayload: StaffJwtPayload | null;
  user: { schoolId: string; role: string; isActive: boolean } | null;
  appRole: string;
  requestSchoolId: string;
}): DaSilvaApplyAuthDecision {
  const payload = input.jwtPayload;
  if (!payload?.userId || !payload?.schoolId) {
    return { allowed: false, status: 401, error: "Authentication required" };
  }
  if (!input.user?.isActive) {
    return { allowed: false, status: 401, error: "Authentication required" };
  }
  if (String(input.user.schoolId) !== String(payload.schoolId)) {
    return { allowed: false, status: 403, error: "School access denied" };
  }

  const appRole = String(input.appRole || "").trim();
  if (!ALLOWED_APPLY_ROLES.has(appRole)) {
    return {
      allowed: false,
      status: 403,
      error: "Owner, Admin or Finance role required for penalty apply",
    };
  }

  if (!isDaSilvaLatePenaltySchoolAllowed(input.user.schoolId)) {
    return {
      allowed: false,
      status: 403,
      error: "Da Silva late penalty apply is not available for this school.",
    };
  }

  const requestSchoolId = String(input.requestSchoolId || "").trim();
  if (!requestSchoolId) {
    return { allowed: false, status: 403, error: "Missing schoolId" };
  }
  if (requestSchoolId !== input.user.schoolId) {
    return {
      allowed: false,
      status: 403,
      error: "Request schoolId does not match authenticated school",
    };
  }

  const auth = { ...payload, appRole };
  return { allowed: true, schoolId: input.user.schoolId, appRole, auth };
}

export async function requireDaSilvaLatePenaltyApply(
  req: DaSilvaLatePenaltyApplyRequest,
  res: Response,
  next: NextFunction
) {
  const payload = verifyStaffJwt(req.headers.authorization);
  const user = payload?.userId
    ? await prisma.user.findUnique({
        where: { id: payload.userId },
        select: { id: true, schoolId: true, role: true, isActive: true },
      })
    : null;

  const meta = user ? await getUserAccessMeta(user.id) : null;
  const appRole = String(meta?.appRole || (user ? appRoleFromPrismaRole(user.role) : "")).trim();
  const body = (req.body ?? {}) as Record<string, unknown>;
  const decision = evaluateDaSilvaLatePenaltyApplyAuth({
    jwtPayload: payload,
    user,
    appRole,
    requestSchoolId: String(body.schoolId || "").trim(),
  });

  if (!decision.allowed) {
    return res.status(decision.status).json({ success: false, error: decision.error });
  }

  req.daSilvaApplyAuth = decision.auth;
  return next();
}
