...
export type ParseResult =
  | { ok: true; format: "csv" | "ofx"; transactions: ParsedBankTransaction[] }
...
export function parseCsvBankStatement(content: string): ParseResult {
  ...
  for (const line of lines.slice(1)) {
    ...
    if (creditIdx >= 0 || debitIdx >= 0) {
      ...
      if (raw.includes("-") || raw.toLowerCase().includes("dr")) moneyOut = amt;
      else moneyIn = amt;
    ...
    if (!date && !description && !moneyIn && !moneyOut) continue;
  ...
  if (!transactions.length) return { ok: false, error: "No transactions found in CSV" };
  return { ok: true, format: "csv", transactions };
}

export function parseOfxBankStatement(content: string): ParseResult {
  const transactions: ParsedBankTransaction[] = [];
  const blocks = content.split(/<STMTTRN>/i).slice(1);

  for (const block of blocks) {
    const dateRaw =
      block.match(/<DTPOSTED>(\d{8})/i)?.[1] ||
  ...
  if (!transactions.length) return { ok: false, error: "No transactions found in OFX file" };
  return { ok: true, format: "ofx", transactions };
}

export function parsePdfBankStatementPlaceholder(): ParseResult {
  return {
    ok: false,
    error:
      "PDF bank statement import is not available yet. Please upload CSV or OFX. OCR integration is pending.",
  };
}
