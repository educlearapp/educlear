...
const [selectedLearnerReport, setSelectedLearnerReport] = useState<any>(null);
          ...
          >
            ...
            {profileTab === "general" ? (
              ...
              >
                ...
                {field("ID No", "idNumber", learner.idNumber)}
  
  
  
                {field("Birth Date", "birthDate", formatDate(learner.birthDate), true, "date")}
                ...
                {field(
                  ...
                  formatDate(learner.enrolmentDate || learner.createdAt),
  ...
  const savedPaymentsByAccount = (accountNo: string) =>
  
  
  
    readJsonArray(`savedPayments:${accountNo}`);
  ...
  `SIL${String(index + 1).padStart(3, "0")}`;
    ...
    const learnerId = String(learner.id || learner.learnerId || accountNo);
    ...
    const learnerInvoices = invoiceRuns.flatMap((run: any) =>
      ...
      Array.isArray(run.rows)
        ...
        ? run.rows.filter((row: any) => {
            ...
            const rowLearnerId = String(row.id || row.learnerId || row.accountNo || "");
    
    
    
            const rowAccountNo = String(row.accountNo || "");
    
    
    
            const rowName = String(row.firstName || "").trim().toLowerCase();
    
    
    
            const rowSurname = String(row.surname || "").trim().toLowerCase();
    ...
    const payments = savedPaymentsByAccount(accountNo);
    ...
    const lastInvoiceRow = learnerInvoices[learnerInvoices.length - 1];
  
  
  
    const lastPaymentRow = payments[0];
    ...
    return {
      ...
      lastInvoice: lastInvoiceRow
        ...
        ? `R ${Number(lastInvoiceRow.invoiceAmount ||



          lastInvoiceRow.amount ||
          
          
          
          lastInvoiceRow.total ||
      ...
      lastInvoiceDate:
        ...
        lastInvoiceRow?.date || lastInvoiceRow?.invoiceDate || lastInvoiceRow?.createdAt || "",
